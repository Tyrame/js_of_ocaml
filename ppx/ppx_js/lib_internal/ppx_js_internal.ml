...
(** Check if an expression is an identifier and returns it. Raise a
    Location.error if it's not. *)
...
(** arg1 -> arg2 -> ... -> ret *)
let arrows args ret =
  List.fold_right (fun (l, ty) fun_ -> Typ.arrow l ty fun_) args ret
...
let inside_Js =
  lazy
    ( try
        Filename.basename (Filename.chop_extension !Location.input_name)
        = "js"
      with Invalid_argument _ -> false )

(* -- FIXME -- [merlin_noloc] is an attempt to hide some ast node from
   merlin when using merlin-type-enclosing-go-{up,down}. It turns out not to
   be working properly, just hiding location instead of the all ast node. *)
...
let unescape lab =
  ...
  then lab
    ...
    let lab =
      if lab.[0] = '_'
      then String.sub lab 1 (String.length lab - 1)
      else lab
    in
    try
...
(* uplift : type of the unused value - ties all types together downlift :
   types of individual components (arguments and result) *)
let invoker ?(extra_types= []) uplift downlift body arguments =
  ...
  (* Build the function. The last arguments is just used to tie all types
     together. It's unused in the implementation. {[ fun (t1 : type_of_t1)
     (t2 : type_of_t2) (_ : uplift_type) -> e]} *)
  let labels_and_pats =
    List.map
      (fun d ->
  ...
  let invoker =
    List.fold_right2 make_fun labels_and_pats tfunc_args
      (make_fun
         ...
         (Label.nolabel, twrap) annotated_ebody)
  in
  (* Introduce all local types: {[ fun (type res t0 t1 ..) arg1 arg2 -> e ]} *)
  let local_types =
    make_str res
    :: List.map (fun x -> make_str (Arg.name x)) (extra_types @ arguments)
  in
...
(* {[ obj##meth x y ]} generates {[ ( fun (type res a2 a0 a1) -> fun (a2 :
   a2 Js.t) -> fun (a0 : a0) -> fun (a1 : a1) -> fun (_ : a2 -> a0 -> a1 ->
   res Js.meth) -> (Js.Unsafe.meth_call a2 "meth" [|(Js.Unsafe.inject a0);
   (Js.Unsafe.inject a1) |] : res) ) (obj : < .. > Js.t) x y (fun x ->
   x#meth) ]} *)
...
(* {[ obj##.prop ]} generates {[ ( fun (type res a0) -> fun (a0 : a0 Js.t)
   -> fun (_ : a0 -> < get :res ;.. > Js.gen_prop) -> (Js.Unsafe.get a0
   "prop" : res) ) (obj : < .. > Js.t) (fun x -> x#prop) ]} *)
...
(* {[ obj##.prop := expr ]} generates {[ ( fun (type res a1 a0) -> fun (a1 :
   a1 Js.t) -> fun (a0 : a0) -> fun (_ : a1 -> < set :a0 -> unit ;.. >
   Js.gen_prop) -> (Js.Unsafe.set a1 "prop" (Js.Unsafe.inject a0) : unit) )
   (obj : < .. > Js.t) expr (fun x -> x#prop) ]} *)
...
(* {[ new%js constr x y ]} generates {[ ( fun (type res a2 a0 a1) -> fun (a2
   : (a0 -> a1 -> res Js.t) Js.constr) -> fun (a0 : a0) -> fun (a1 : a1) ->
   fun (_ : unit) -> (Js.Unsafe.new_obj a2 [|(Js.Unsafe.inject a0);
   (Js.Unsafe.inject a1) |] : res Js.t) ) constr x y () ]} *)
...
let new_object constr args =
    ...
    invoker
      ...
      (fun args tres ->
        ...
        | unit :: args ->
            ...
            ( (Label.nolabel, Js.type_ "constr" [arrows args tres]) :: args
            , tres ) )
      (function
  ...
  Exp.apply invoker
    ...
    @ [ app_arg
          (Exp.construct ~loc:constr.loc (lid ~loc:constr.loc "()") None) ]
    )

module S = Map.Make (String)

(** We remove Pexp_poly as it should never be in the parsetree except after
    a method call. *)
let format_meth body =
  match body.pexp_desc with Pexp_poly (e, _) -> e | _ -> body

(** Ensure basic sanity rules about fields of a literal object: - No
    duplicated declaration - Only relevant declarations (val and method, for
    now). *)

module Prop_kind = struct
  type t = [`Readonly | `Writeonly | `Readwrite | `Optdef]
...
let preprocess_literal_object mappper fields :
  ...
  let check_name id names =
    ...
    then
      ...
      (* We point out both definitions in locations (more convenient for the
         user). *)
      let details id =
        if id.txt <> txt
      ...
      in
      let sub =
        [ Location.errorf ~loc:id'.loc "Duplicated val or method %S%s."
            id'.txt (details id') ]
      in
  ...
  in
  try `Fields (List.rev (snd (List.fold_left f (S.empty, []) fields)))
  with Location.Error error -> `Error (extension_of_error error)

(* {[ object%js (self) val readonlyprop = e1 val prop = e2 method meth x =
   e3 end ]} generates {[ ( fun (type res a6 a7 a8 a9) -> fun (a7 : a7) ->
   fun (a8 : a8) -> fun (a9 : res Js.t -> a6 -> a9) -> fun (_ : res Js.t ->
   a7 Js.readonly_prop -> a8 Js.prop -> (res Js.t -> a6 -> a9 Js.meth) ->
   res) -> (Js.Unsafe.obj [|("readonlyprop", (Js.Unsafe.inject a7));
   ("prop", (Js.Unsafe.inject a8)); ("meth", (Js.Unsafe.inject
   (Js.wrap_meth_callback a9))) |] : res Js.t) ) e1 e2 (fun self -> fun x ->
   e3) (fun self read_only_prop prop meth -> object method read_only_prop =
   read_only_prop method prop = prop method meth = meth self end) ]} *)
let literal_object self_id (fields: field_desc list) =
  ...
  let extra_types =
    List.concat
      (List.map (function Val _ -> [] | Meth (_, _, _, _, l) -> l) fields)
  in
  let invoker =
    invoker ~extra_types
      (fun args tres ->
            ...
            (fun f ->
              ...
              let apply e =
                ...
                | Meth _ ->
                    Exp.apply e
                      [ ( Label.nolabel
                        , Exp.ident (lid ~loc:Location.none self) ) ]
              in
              { pcf_loc= loc
              ; pcf_attributes= []
              ; pcf_desc=
                  Pcf_method
                    ...
                    , Cfk_concrete
                        ...
                        , apply
                            (Exp.ident ~loc
                               (lid ~loc:Location.none (name f).txt)) ) ) }
              )
            fields }
  in
  Exp.apply invoker
    ...
    expr=
      (fun mapper expr ->
        ...
        let new_expr =
          ...
          (* obj##(meth arg1 arg2) .. *)
          | [%expr
              [%e ? obj] ## [%e ? {pexp_desc= Pexp_apply (meth, args); _}]] ->
              let meth_str = exp_to_string meth in
              let obj = mapper.expr mapper obj in
              let args =
                List.map (fun (s, e) -> s, mapper.expr mapper e) args
              in
              let new_expr =
                method_call ~loc:meth.pexp_loc obj meth_str args
              in
              mapper.expr mapper {new_expr with pexp_attributes}
          (* obj##meth arg1 arg2 .. *)
          | { pexp_desc=
                Pexp_apply
                  (([%expr [%e ? obj] ## [%e ? meth]] as prop), args); _ } ->
              let meth_str = exp_to_string meth in
              let obj = mapper.expr mapper obj in
              let args =
                List.map (fun (s, e) -> s, mapper.expr mapper e) args
              in
              let new_expr =
                method_call ~loc:prop.pexp_loc obj meth_str args
              in
              mapper.expr mapper {new_expr with pexp_attributes}
          (* obj##meth *)
          ...
          | [%expr [%js [%e ? {pexp_desc= Pexp_object class_struct; _}]]] ->
              let fields =
                preprocess_literal_object (mapper.expr mapper)
...
let () =
  Driver.register ~name:"ppx_js" Migrate_parsetree.Versions.ocaml_406
