...
let check_error gl =
  if gl##getError <> gl##._NO_ERROR_ then error "WebGL error"
...
let getfile f =
  try Lwt.return (Sys_js.read_file ~name:f) with Not_found ->
    http_get f >|= fun s -> s
...
let start (pos, norm) =
  ...
  let ambientLight_loc =
    gl##getUniformLocation prog (string "u_ambientLight")
  in
  ...
  let mat =
    ...
    mult
      (rotate_x (pi /. 2.))
      (mult (scale 0.5 0.5 0.5) (translate 0. 0. 0.))
  in
  check_error gl ;
  debug "ready" ;
