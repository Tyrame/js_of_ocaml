...
let loading_style =
  js
    "background-color: red; color: white; display:inline; position: \
...
let rec fall state =
  ...
  if !changed
  then Lwt_js.sleep 0.05 >>= fun () -> fall state
  else Lwt.return ()
  ...
  let rec update (x, y) next img over_cont out_cont click_cont =
    ...
    then (
      ...
      and click' () =
        ...
        Lwt_js.sleep 0.05
        >>= fun () ->
        fall state >>= fun () -> set_cell state x y Empty ; Lwt.return ()
      in
      let click () =
        ...
        Lwt.catch
          (fun () ->
            click_cont ()
            >>= fun () ->
            if ((state.map).(y)).(x) = Diamond
            then state.rem <- state.rem - 1 ;
            set_cell state x y Guy ;
            state.pos <- x, y ;
            fall state )
          (fun e ->
      ...
      if ((state.map).(y)).(x) <> End
      then update (next (x, y)) next img over out click' )
  in
...
let start _ =
  ...
  let load_data name process =
    ...
    getfile name
    >>= fun data ->
    process data >>= fun res -> loading_end () ; Lwt.return res
  in
  ...
  body##.style##.cssText
  := js
       "font-family: sans-serif; text-align: center; background-color: \
        #e8e8e8;" ;
  ...
  List.iter
    (fun (_f, n) ->
      ...
      (* option##onclick <- some (fun _ -> ignore (load_level f);
         Js._false); *)
      Dom.appendChild select option )
    levels ;
  select##.onchange :=
