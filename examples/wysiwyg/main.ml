...
let rec html2wiki body =
  ...
  let add_str ?(surr= "") s =
    if is_visible_text s
    then Buffer.add_string ans (surr ^ s ^ surr)
    else ()
  in
...
let onload _ =
  ...
  let body =
    Js.Opt.get
      (d##getElementById (Js.string "wiki_demo"))
      (fun () -> assert false)
  in
  ...
  Js.Opt.iter iframe##.contentDocument (fun iDoc ->
      ...
      let createButton ?(show= Js._false) ?(value= None) title action =
        ...
        let wrap s =
          match s with None -> Js.null | Some s -> Js.some (Js.string s)
        in
        but##.onclick :=
          Html.handler (fun _ ->
              iWin##focus ;
      ...
      := Html.handler (fun _ ->
             ...
             iDoc##execCommand (Js.string "inserthtml") Js._false
        ...
        let n =
          ...
          then (
            ( try
                ...
                wikiFrame##.value :=
                  Js.string (html2wiki (iDoc##.body :> Dom.node Js.t))
              with _ -> () ) ;
            20 )
          else max 0 (n - 1)
        in
