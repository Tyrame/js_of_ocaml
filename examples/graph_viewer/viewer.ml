...
module Common = Viewer_common.F (struct
  ...
  let perform_draw ctx fill_color stroke_color =
    (match fill_color with Some c -> fill ctx c | None -> ()) ;
    match stroke_color with Some c -> stroke ctx c | None -> ()
...
let scroll_view ?width ?height ?packing st =
  ...
  let sadj =
    GData.adjustment ~upper:20. ~step_incr:1. ~page_incr:1. ~page_size:0. ()
  in
  let zoom_steps = 8. in
  ...
  ignore
    (((display#event)#connect)#expose (fun ev ->
         ...
         redraw st (get_scale ()) hadj#value vadj#value display
           (display#misc)#allocation x y width height ;
         true )) ;
  ignore
    ((hadj#connect)#value_changed (fun () ->
         GtkBase.Widget.queue_draw display#as_widget )) ;
  ignore
    ((vadj#connect)#value_changed (fun () ->
         GtkBase.Widget.queue_draw display#as_widget )) ;
  ...
  ignore
    ((sadj#connect)#value_changed (fun () ->
         ...
         hadj#set_value
           (hadj#value +. (hadj#page_size *. r *. fst !zoom_center)) ;
         vadj#set_value
           (vadj#value +. (vadj#page_size *. r *. snd !zoom_center)) ;
         prev_scale := scale ;
         refresh () ;
         update_scrollbars () )) ;
  ...
  ignore
    (((display#event)#connect)#key_press (fun ev ->
         ...
         else if keyval = GdkKeysyms._minus
...
let create ?(full_screen= false) (x1, y1, x2, y2) scene =
  ...
  let f =
    scroll_view ~width:initial_size ~height:initial_size
      ~packing:(b#pack ~expand:true) st
  in
  ignore
    (GRange.scale `VERTICAL ~inverted:true ~draw_value:false
       ~adjustment:f#scale_adjustment ~packing:b#pack ()) ;
  (*XXX Tooltips area#misc#set_has_tooltip true; ignore
    (area#misc#connect#query_tooltip (fun ~x ~y ~kbd tooltip ->
  ...
  ignore
    (((w#event)#connect)#key_press (fun ev ->
         ...
         then exit 0
         else if keyval = GdkKeysyms._F11 || keyval = GdkKeysyms._F5
