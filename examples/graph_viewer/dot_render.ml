...
let parse_float s =
  try float_of_string s with Failure _ -> raise Not_found
...
let render_spline l =
  match l with
  | (x, y) :: r ->
      Array.of_list (Scene.Move_to (x, y) :: render_spline_rec r)
  | _ -> raise Not_found
...
let parse_spline scene s color arrow_size =
  ...
  List.iter
    (fun s ->
      let l =
        List.map (fun s -> Str.split comma_re s) (Str.split wsp_re s)
      in
      let endp, l =
        match l with
...
let f g =
  ...
  let margin =
    try parse_float (StringMap.find "margin" g.G.graph_attr)
    with Not_found -> 4.
  in
  ...
  IntMap.iter
    (fun _ n ->
      ...
      let width =
        dpi *. parse_float (StringMap.find "width" n.G.node_attr)
      in
      let height =
        dpi *. parse_float (StringMap.find "height" n.G.node_attr)
      in
      let color =
        parse_color
          ( try StringMap.find "color" n.G.node_attr with Not_found ->
              "black" )
      in
      let shape =
      ...
      (*XXX parse style *)
      let style =
        try StringMap.find "style" n.G.node_attr with Not_found -> ""
      in
      let fillcolor =
        if style <> "filled"
        then None
      ...
      in
      let font_size =
        try parse_float (StringMap.find "fontsize" n.G.node_attr)
        with Not_found -> 14.
      in
      let label =
        (*XXX Parse...*)
  ...
  IntMap.iter
    (fun _ e ->
      ...
      let color =
        parse_color
          ( try StringMap.find "color" e.G.edge_attr with Not_found ->
              "black" )
      in
      let arrow_size =
        try parse_float (StringMap.find "arrowsize" e.G.edge_attr)
        with Not_found -> 1.
      in
      parse_spline scene
        (StringMap.find "pos" e.G.edge_attr)
        color arrow_size ;
      () )
    g.G.edges.G.seq ;
  bbox, scene
