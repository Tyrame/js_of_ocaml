...
type 'a sequence =
  {mutable count: int; mutable seq: 'a IntMap.t; id: (id, 'a) Hashtbl.t}
...
type node = {name: string; id: id; mutable node_attr: string StringMap.t}
...
let rec all_parents s g =
  ...
  then s
    ...
    Hashtbl.fold
      (fun _ g s -> all_parents s g)
      g.parents
      (IntMap.add g.graph_id g s)
...
let opt_add nm v m =
  match v with Some v -> StringMap.add nm v m | None -> m
...
let rec add_edges st g x r key attrs =
  ...
  | y :: r ->
      ...
      IntMap.iter
        (fun _ n1 ->
          IntMap.iter (fun _ n2 -> add_edge st g n1 p1 n2 p2 key attrs) s2
          )
        s1 ;
  ...
  let c =
    List.map
      (fun s ->
        ...
        | `Node node ->
            `Node
              ( find_node st g node.Dot_file.name def_attr
              , node.Dot_file.port )
