...
let draw_element ctx e =
  ...
  | Text (x, y, txt, font, font_size, fill, stroke) ->
      ...
      Cairo.move_to ctx
        (x -. ext.Cairo.x_bearing -. (ext.Cairo.text_width /. 2.))
        y ;
      Cairo.select_font_face ctx font Cairo.FONT_SLANT_NORMAL
...
let read_polygon attrs i =
  ...
  let points =
    group (List.map float_of_string (Str.split comma_wsp points))
  in
  let fill = parse_color (List.assoc fill_attr attrs) in
  let stroke = parse_color (List.assoc stroke_attr attrs) in
...
let read_text attrs i =
  let fill =
    parse_color (try List.assoc fill_attr attrs with Not_found -> "black")
  in
  let stroke =
...
let rec read_element nm attrs i =
  ...
  | `El_start ((_, nm'), attrs') ->
      (* Format.eprintf "%s" nm'; List.iter (fun ((_, nm), _) ->
         Format.eprintf " %s" nm) attrs'; Format.eprintf "@."; *)
      ( match nm' with
...
let redraw w range ev =
  ...
  in
  (* let (x1, y1, x2, y2) = bbox in Format.eprintf "%f %f %f %f (%f)@." x1
     y1 x2 y2 scale; *)
  List.iter2
    (fun box e -> if intersects box bbox then draw_element ctx e)
    !bboxes l ;
  Cairo.restore ctx ;
  (* let t2 = Unix.gettimeofday () in Format.eprintf "%f@." (t2 -. t1); *)
  true
...
let _ =
  ...
  (* let f = GBin.frame ~shadow_type:`IN ~packing:(b#pack ~expand:true
     ~fill:true) () in *)
  let f =
  ...
  in
  let area =
    GMisc.drawing_area ~width:initial_size ~height:initial_size
  ...
  (area#misc)#set_size_request
    ...
    () ;
  let slider =
    GRange.scale `HORIZONTAL ~draw_value:false ~packing:b#pack ()
  in
  (slider#adjustment)#set_bounds ~lower:0. ~upper:1. ~step_incr:0.1 () ;
  (* let button = GButton.check_button ~label:"Animate" ~packing:b#pack ()
     in

     ignore (area#event#connect#expose (redraw area slider)) ; ignore
     (slider#connect#value_changed (slider_changed area)) ; ignore
...
(* let _ = let l = List.rev !stack in Format.eprintf "len: %d@."
   (List.length l); let t1 = Unix.gettimeofday () in List.iter (fun e ->
   draw_element ctx e) l; let t2 = Unix.gettimeofday () in Format.eprintf
   "%f@." (t2 -. t1);

   (* let ch = open_out "/tmp/foo.mar" in Marshal.to_channel ch l [];
   close_out ch; *) Cairo_png.surface_write_to_file s "/tmp/foo.png" *)
