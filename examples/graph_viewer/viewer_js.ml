...
module Common = Viewer_common.F (struct
  ...
  let put_pixmap ~dst:((_p, c): drawable) ~x ~y ~xsrc ~ysrc ~width ~height
    ...
    c##drawImage_fullFromCanvas
      p (float xsrc) (float ysrc) (float width) (float height) (float x)
...
(* ;Firebug.console##timeEnd (Js.string "draw") ;Firebug.console##log_2
   (Js.string "draw", Js.date##now()) *)
          ...
          Js.some
            (Html.addEventListener Html.document Html.Event.mouseup
               (Html.handler (fun _ ->
                    ...
                    Js._true ))
               Js._true) ;
        (* We do not want to disable the default action on mouse down (here,
           keyboard focus) in this example. *)
        Js._true )
...
let start () =
  ...
  let sadj =
    new adjustment ~upper:20. ~step_incr:1. ~page_incr:0. ~page_size:0. ()
  in
  let zoom_steps = 8. in
  ...
  let update_view _force =
    ...
    (* if force then redraw st (get_scale ()) hadj#value vadj#value canvas
       else if not !redraw_queued then ignore (redraw_queued := true; (*
       Firebug.console##log(Js.string "sleep"); *) Lwt_js.yield() >>= fun ()
       -> redraw_queued := false; redraw st (get_scale ()) hadj#value
       vadj#value canvas; Lwt.return ()) *)
  in
  let a = allocation () in
  let zoom_factor =
  ...
  let adjust_slider () =
    let pos' =
      height - truncate ((sadj#value *. float height /. sadj#upper) +. 0.5)
    in
    thumb##.style##.top := points pos' ;
    pos := pos'
  in
  ...
  handle_drag canvas (fun dx dy ->
      ...
      let offset a d =
        a#set_value
          (min (a#value -. (float d /. scale)) (a#upper -. a#page_size))
      in
  ...
  let bump_scale x y v =
    ...
    let vl =
      min sadj#upper (max sadj#lower (prev +. (v *. sadj#step_increment)))
    in
    if vl <> prev
    then (
      sadj#set_value vl ;
      ...
      if x >= 0. && x <= 1. && y >= 0. && y <= 1.
      then rescale x y
      else rescale 0.5 0.5 ) ;
    Js._false
  in
  (* Zoom using the mouse wheel *)
  ignore
    (Html.addMousewheelEventListener canvas
       (fun ev ~dx:_ ~dy ->
       ...
       Js._true) ;
  (* Html.addEventListener Html.document Html.Event.keydown (Html.handler
     (fun e -> Firebug.console##log(e##keyCode); Js._true)) Js._true; *)
  (* Html.addEventListener Html.document Html.Event.keypress (Html.handler
     (fun e -> Firebug.console##log(Js.string "press"); match e##keyCode
     with | 37 -> (* left *) Js._false | 38 -> (* up *) Js._false | 39 -> (*
     right *) Js._false | 40 -> (* down *) Js._false | _ ->
     Firebug.console##log(- 1- e##keyCode); Js._true)) Js._true; *)
