...
struct
  ...
  let grow_pixmap pm window width height =
    ...
    then (
      ...
      ( match old_p with
      | Some old_p ->
          put_pixmap ~dst:(drawable_of_pixmap p) ~x:0 ~y:0 ~xsrc:0 ~ysrc:0
            ~width:r.width ~height:r.height old_p
      | None -> () ) ;
      pm.pixmap <- Some p ;
      pm.p_width <- width ;
      pm.p_height <- height )
  ...
  let get_pixmap pm =
    match pm.pixmap with Some p -> p | None -> assert false
  ...
  let intersects ((x1, y1, x2, y2): float * float * float * float)
      (x3, y3, x4, y4) =
  ...
  let redraw st scale x y x' y' w h =
    ...
    begin_path ctx ;
    rectangle ctx ~x:(float x') ~y:(float y') ~width:(float w)
      ~height:(float h) ;
    M.fill ctx M.white ;
    clip ctx ;
  ...
  let redraw st scale x0 y0 window a x y width height =
    ...
    (* Firebug.console##log_6 (dx, pm.valid_rect.width, a.width, dy,
       pm.valid_rect.height, a.height); *)
    ...
    then (
      ...
      (*XXX FIX: does not change pm.valid_rect when it is large enough
        already and valid *)
      ...
      if dx <> 0 || dy <> 0
      then
        put_pixmap ~dst:(drawable_of_pixmap p) ~x:dx ~y:dy ~xsrc:0 ~ysrc:0
          ~width:r.width ~height:r.height p ;
      let offset p l d m =
        (* 0 <= p; 0 <= l; p + l <= m *)
        if p + d + l <= 0
        then 0, 0
      ...
      then
        ...
        then (
          ...
          assert (x = 0) ;
          if a.width > width
          then redraw st scale x0 y0 width y (a.width - width) height ) ;
      if y > 0
      then (
    ...
    then (
      ...
      pm.valid_rect <- {x= x0; y= y0; width= a.width; height= a.height} ) ;
    put_pixmap ~dst:(get_drawable window) ~x ~y ~xsrc:x ~ysrc:y ~width
      ~height (get_pixmap pm)
end
