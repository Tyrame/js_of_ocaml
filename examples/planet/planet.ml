 ...
 *)
(* - stop animation when not needed ==> not visible ==> no change (paused,
   follow rotation and no lighting) - Options: ==> larger/smaller -
   adaptative size ==> time 3 frames and take min ==> if fast, try larger
   image

   IDEAS ===== - saisons - satellites: geostationnaires, différentes
   altitudes ==> trajectoire + mouvement du satellite - affiche l'axe de
   rotation de la terre, la direction du soleil - autres planètes

   Sphere tessellation... http://sol.gfxile.net/sphere/index.html
   http://www.nihilogic.dk/labs/canvas3dtexture_0.2/

   Stop animation when not visible! ===> use window.onfocus/onblur
...
let radio txt name checked action =
  let b =
    Dom_html.createInput ~name:(Js.string name) ~_type:(Js.string "radio")
      doc
  in
  b##.checked := Js.bool checked ;
  b##.onclick := Dom_html.handler (fun _ -> action () ; Js._true) ;
...
let divide all o =
  ...
  let fn =
    if all then 4 * Array.length o.faces else Array.length o.faces + 24
  in
  ...
  let midpoint v1 v2 =
    ...
    try Hashtbl.find midpoints p with Not_found ->
      ...
      in
      let v =
        if all || abs_float v1.y = 1. || abs_float v2.y = 1.
        then normalize v
        else v
      in
      let res = !j in
...
let shadow texture =
  ...
  let update_shadow obliquity =
    ...
    for j = 0 to h - 1 do
      for i = 0 to (w / 2) - 1 do
        ...
        let k' =
          truncate (4. *. (float w -. float i +. (float j *. float w) -. 1.))
        in
        ...
        in
        let c =
          if x > 0. then dark else dark -. (x *. (1. -. dark) *. 1.2)
        in
  ...
  let update_texture lighting phi =
    ...
    then (
      ...
      let i =
        truncate
          (mod_float (((2. *. pi) -. phi) *. float w /. 2. /. pi) (float w))
      in
  ...
  Array.iteri
    (fun i {v1; v2; v3} ->
      ...
      then (
        ...
        (* let (u1, v1) = uv.(v1) in let (u2, v2) = uv.(v2) in let (u3, v3)
           = uv.(v3) in let mid = tw /. 2. in

           let u1 = if u1 = 0. && (u2 > mid || u3 > mid) then tw -. 2. else
           u1 in let u2 = if u2 = 0. && (u1 > mid || u3 > mid) then tw -. 2.
           else u2 in let u3 = if u3 = 0. && (u2 > mid || u1 > mid) then tw
           -. 2. else u3 in

           let mth = th -. 2. in let u1 = if v1 = 0. || v1 >= mth then (u2
           +. u3) /. 2. else u1 in let u2 = if v2 = 0. || v2 >= mth then (u1
           +. u3) /. 2. else u2 in let u3 = if v3 = 0. || v3 >= mth then (u2
           +. u1) /. 2. else u3 in

           let u1 = max 1. u1 in let u2 = max 1. u2 in let u3 = max 1. u3 in

           let v1 = max 1. v1 in let v2 = max 1. v2 in let v3 = max 1. v3 in

           let du2 = u2 -. u1 in let du3 = u3 -. u1 in let dv2 = v2 -. v1 in
           let dv3 = v3 -. v1 in let dx2 = x2 -. x1 in let dx3 = x3 -. x1 in
           let dy2 = y2 -. y1 in let dy3 = y3 -. y1 in let a =
           (dx2*.dv3-.dx3*.dv2) /. (du2*.dv3-.du3*.dv2) in let b =
           (dx2*.du3-.dx3*.du2) /. (dv2*.du3-.dv3*.du2) in let c = x1 -. a
           *. u1 -. b *. v1 in let d = (dy2*.dv3-.dy3*.dv2) /.
           (du2*.dv3-.du3*.dv2) in let e = (dy2*.du3-.dy3*.du2) /.
           (dv2*.du3-.dv3*.du2) in let f = y1 -. d *. u1 -. e *. v1 in

           ctx##transform (a, d, b, e, c, f); let u = max 0. (min u1 (min u2
           u3) -. 4.) in let v = max 0. (min v1 (min v2 v3) -. 4.) in

           let u' = min tw (max u1 (max u2 u3) +. 4.) in let v' = min th
           (max v1 (max v2 v3) +. 4.) in let du = u' -. u in let dv = v' -.
           v in *)
...
let start _ =
  Lwt.ignore_result
    ...
    in
    let face_info =
      Array.map (fun f -> precompute_mapping_info tw th uv f) o.faces
    in
    ...
    add d
      (doc##createTextNode (Js.string "Click and drag mouse to rotate.")) ;
    add ctrl d ;
    ...
    ( add form (toggle_button "Pause" "Resume" (fun p -> paused := p)) ;
      add form (br ()) ;
      add form
        (toggle_button "Follow rotation" "Fixed position" (fun f ->
             follow := f )) ;
      add form (br ()) ;
      add form
    ...
    p##.innerHTML :=
      Js.string
        "Credit: <a href='http://visibleearth.nasa.gov/'>Visual Earth</a>, \
         Nasa" ;
    add doc##.body p ;
    let mx = ref 0 in
    let my = ref 0 in
            ...
            Html.addEventListener Html.document Html.Event.mousemove
              (Dom_html.handler (fun ev ->
                   ...
                   then
                     m :=
                       matrix_mul
                         (yz_rotation (2. *. float dy /. float width))
                         !m ;
                   if dx != 0
                   then
                     m :=
                       matrix_mul
                         (xz_rotation (2. *. float dx /. float width))
                         !m ;
                   mx := x ;
                   my := y ;
                   Js._true ))
