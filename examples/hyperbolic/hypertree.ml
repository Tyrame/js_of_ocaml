...
(* - check compatibility + wait page
   ...
   - files to load set in index.html ==> tree.txt, icon directory,
   image_info.json, image directories

   - validation tools

   - we should stop updating the canvas when it is not visible (opaque
   overlay)

   - do not cache images forever (just keep a list of recently used images)
   ==> keep images used last + twice as many / minimum 200 ?

   - spinner: draw it on a canvas (===> large canvas...)

   - webgl?

   - swap image once loaded instead of putting it over the canvas (?)

   - récupérer le nom des animaux? ==> overlay mechanism: save what is
   below, display overlay; restore when moved

   - can we have fast shadows? ==> precompute the shadows in a canvas? -
   should we have half size images for better rendering??? (might make it
   possible fast shadows?)

   - support for touch events - limit how far we can go

   - use history to save current location and random seed

   - get all image sizes so that we can size precisely images before they
   are all loaded

   ==============

   - find point closest to center; start drawing from it; stop below a given
   threshold - speed-up rendering by traversing the tree and stopping when
   the edge length becomes below some thresold

   ===============

   - preferred diameter of each node spring-like forces around this diameter
   repulsive forces: exponential when below the preferred diameter decrease
   exponentially
   http://en.wikipedia.org/wiki/Hyperbolic_geometry#Circles.2C_spheres_and_balls

   - importante répulsion quand trop près! ==> surtout pour feuilles! -
   cap the acceleration

   - metric tree/vp-tree? r trees / kd trees *)
...
class type style =
  object
    ...
    method nodeBackgroundColor :
      Js.js_string Js.t Js.optdef Js.readonly_prop
...
let mul z t =
  {x= (z.x *. t.x) -. (z.y *. t.y); y= (z.x *. t.y) +. (z.y *. t.x)}
...
let hyp_transf_vect (p, t) v v' =
  ...
  let c = mul (conj p) t in
  let d = one in
  for i = 0 to Array.length v - 1 do v'.(i) <- transf a b c d v.(i) done
...
(* Transformation from z0 to z1: z1 = (z0 + p) / (conj p.z0 + 1) ==> p =
   (z1.z0.conj (z1 - z0) + z1 - z0) / (1 - |z1.z0|^2) *)
...
let json : <parse: Js.js_string Js.t -> 'a> Js.t =
...
(* let d = Html.document in Dom.appendChild d##body debug_widget; let p =
   Html.createP d in p##innerHTML <- Js.string s; Dom.appendChild
   debug_widget p *)
        ...
        let c1 =
          Html.addEventListener Html.document Html.Event.mousemove
            (Html.handler (fun ev ->
                 ...
                 (* debug_msg (Format.sprintf "Mouse move %d %d %d %d" x0 y0
                    x y); *)
                 if not !started
        ...
        c2 :=
          Js.some
            (Html.addEventListener Html.document Html.Event.mouseup
               (Html.handler (fun ev ->
                    (* debug_msg (Format.sprintf "Mouse up %d %d %d %d" x0
                       y0 ev##clientX ev##clientY); *)
...
let handle_touch_events element move stop cancel click =
  let fuzz = 4 in
  ignore
    (Html.addEventListener element Html.Event.touchstart
       (Html.handler (fun ev ->
            Js.Optdef.iter
                         ...
                         for i = 0 to ev##.changedTouches##.length - 1 do
                           Js.Optdef.iter
                             ...
                             (fun touch ->
                               ...
                               then (
                                 let x = touch##.clientX
                                 and y = touch##.clientY in
                                 (* debug_msg (Format.sprintf "Touch move %d
                                    %d %d %d" x0 y0 x y); *)
                                 if not !started
                                    && ( abs (x - x0) > fuzz
                                       || abs (y - y0) > fuzz )
                              ...
                              Js.Optdef.iter
                                ...
                                (fun touch ->
                                  ...
                                  then (
                                    let x = touch##.clientX
                                    and y = touch##.clientY in
                                    (* debug_msg (Format.sprintf "Touch
                                       cancel %d %d %d %d" x0 y0 x y); *)
                                    Html.removeEventListener c1 ;
                                    Js.Opt.iter !c2 Html.removeEventListener ;
                                    Js.Opt.iter !c3 Html.removeEventListener ;
                                    if !started
...
(* let handle_touch_events element move stop cancel = let fuzz = 4 in let id
   = ref 0 in let x0 = ref 0 in let y0 = ref 0 in ignore
   (Html.addEventListener element Html.Event.touchstart (Html.handler (fun
   ev -> OptDef.iter (ev##changedTouches##item(0)) (fun touch -> id :=
   touch##identifier; in x0 := touch##clientX; y0 := touch##clientY)); (* x0
   := ev##pageX; y0 := ev##pageY;*) debug_msg (Format.sprintf "Touch down %d
   %d" !x0 !y0); Js._false)) Js._false); ignore (Html.addEventListener
   element Html.Event.touchmove (Html.handler (fun ev -> (* let x =
   ev##pageX and y = ev##pageY in *) debug_msg (Format.sprintf "Touch move
   %d %d %d %d" !x0 !y0 x y); move !x0 !y0 x y; Js._false)) Js._false);
   ignore (Html.addEventListener element Html.Event.touchend (Html.handler
   (fun _ -> debug_msg (Format.sprintf "Touch end"); stop (); Js._false))
   Js._false); ignore (Html.addEventListener element Html.Event.touchcancel
   (Html.handler (fun _ -> cancel (); Js._true)) Js._false) *)
...
let load_messages () =
  ...
  >>= fun s -> Lwt.return (json##parse (Js.string s))

let local_messages msgs : messages Js.t =
  option (Js.Unsafe.get msgs !language)

(******)

let screen_transform canvas =
  let offset =
    opt_style style##.border 0.5 +. opt_style style##.padding 0.
  in
...
(* We have |z0|^2 = 1 + r^2 (the "line" is orthogonal to the unit circle)
   |z1-z0|^2 = r^2 (z1 is on the circle) |z2-z0|^2 = r^2 (z2 is on the
   circle) By solving this set of equations, we get z0. *)
...
let draw canvas vertices edges nodes boxes =
  ...
  for i = 0 to Array.length nodes - 1 do
    ...
    | `Img (img, _) -> (
        ...
        then (
          ...
          | Some img ->
              ...
              (* let s = min_scale l w h (1. /. min w h) in let scale = s *.
                 r in *)
              let s = min_scale l 1. 1. 1. in
              ...
              if w > 1. && h > 1.
              then (
                (* let ratio = r *. min (min_w /. w) (min_h /. h) in let w =
                   w *. ratio in let h = h *. ratio in *)
                if shadow
                then (
                  c##save ;
                  (* let blur = 7. /. img_r *. rd in let offset = 5. /.
                     img_r *. rd in *)
                  let blur = 7. *. scale in
                ...
                let x = (z.x *. rx) +. dx in
                let y = (z.y *. ry) +. dy in
                c##drawImage_withSize img (x -. w) (y -. h) (2. *. w)
                  (2. *. h) ;
                (* c##drawImage_fromCanvasWithSize (img, x -. w, y -. h, 2.
                   *. w, 2. *. h); *)
    ...
    | `Txt (circle, Some txt, _) ->
        ...
        let r2 =
          Array.fold_left
            (fun r2 (j, _) -> min r2 (sq_norm (sub vertices.(j) z)))
            1. l
        in
...
(* let load_image src = load_image src >>= fun img -> let w = img##width in
   let h = img##height in let canvas = create_canvas (w + 14) (h + 14) in
   let c = canvas##getContext (Html._2d_) in c##shadowBlur <- 7.;
   c##shadowOffsetX <- 5.; c##shadowOffsetY <- 5.; c##shadowColor <-
   Js.string "black"; c##drawImage_withSize (img, 2., 2., float w, float h);
   Lwt.return canvas *)
...
let compute_text_nodes node_names nodes =
  ...
  for i = 0 to Array.length nodes - 1 do
    ...
    | neigh, `Txt (is_root, _, info) ->
        let canvas =
          try Some (compute_text_node (Hashtbl.find names info))
          with Not_found -> None
        in
...
let compute_neighbors nodes tree =
  ...
  let rec compute_frontiers node =
    ...
    (* (i :: fst frontiers.(node_info (List.hd l)), i :: snd
       frontiers.(node_info (list_tl l))) *)
  in
...
let array_map2 f a1 a2 =
  ...
  then [||]
    ...
    for i = 1 to l - 1 do r.(i) <- f a1.(i) a2.(i) done ;
    r
...
let img_button ?href h src =
  ...
  let decoration over =
    ...
    div##.style##.padding
    := Js.string
         (* (Format.sprintf "%dpx 3px %dpx 3px" (extra / 2) (extra - extra /
            2)); *)
         ...
         ^ "px 3px" ) ;
    div##.className :=
      Js.string ("filled_button " ^ if over then "on" else "off") ;
...
let show_image all_messages image_info name small_image =
  ...
  (* let suffix_re = Regexp.regexp "\\.[^.]*$" in let name = Js.string
     (Regexp.global_replace suffix_re name "") in *)
  let i = ref (-1) in
  ...
  then (
    ...
    let wrap elt =
      ...
      (* w##style##width <- Js.string "100%"; w##style##height <- Js.string
         "100%"; w##style##maxWidth <- Js.string (string_of_int info.width ^
         "px"); *)
    ...
    | Some small_image ->
        ...
        let c = canvas##getContext Html._2d_ in
        c##drawImage_withSize small_image 0. 0. (float info.width)
          (float info.height) ;
    ...
    (* let p = Html.createP d in p##innerHTML <- Js.string ("See the <a
       target=\"_blank\" href=\"" ^ Js.to_string info.url ^ "\">image \
       description page</a> on Wikimedia Commons for more \ information.");
    ...
    let url =
      let suffix =
        if !language == Js.string "en"
        then ""
        else "?uselang=" ^ Js.to_string !language
      in
      "http://commons.wikimedia.org/wiki/File:" ^ Js.to_string info.url
      ^ suffix
    in
    ...
    if !language != Js.string "en"
    then list (Js.string "In English") (Js.string "en") ;
    if !count > 0
...
let information_en =
  Js.string
    "<h1>A tree of animals</h1> This <a target=\"_blank\" \
     ...
     tree</a> displays the relationships among \
     animals.<h2>Usage</h2>Browse the tree by dragging it with the mouse. \
     Click on any image to display a larger version.<h2>Credits</h2>This \
     software and the images it uses are under free licenses. Images are \
     ...
     has been written by Jérôme Vouillon (CNRS)."
...
let unsupported_messages () =
  ...
  txt##.innerHTML :=
    Js.string
      "Unfortunately, this browser is not supported. Please try again with \
       another browser, such as <a \
       href=\"http://www.mozilla.org/firefox/\">Firefox</a>, <a \
  ...
  Lwt.ignore_result
    ...
    (redraw_funct :=
       fun () ->
         ...
         (* let transf = hyp_transf !tr' in for i = 0 to Array.length
            vertices - 1 do vertices'.(i) <- transf vertices.(i) done; *)
         let w = page##.clientWidth in
         let h = page##.clientHeight in
    ...
    Html.window##.onresize :=
      Html.handler (fun _ ->
          ...
          if w <> canvas##.width || h <> canvas##.height
          then
            (* canvas##width <- w; canvas##height <- h; perform_redraw () *)
            schedule_redraw () ;
          Js._true ) ;
    (* let eventually t f = let scheduled = ref false in fun () -> if not
       !scheduled then begin scheduled := true; ignore (Lwt_js.sleep t >>=
       fun () -> scheduled := false; f (); Lwt.return ()) end in *)
    ...
    handle_drag canvas
      (fun x0 y0 x1 y1 ->
        ...
        (* (* Transformation from z0 to z1: z1 = (z0 + p) / (conj p.z0 + 1)
           ==> p = (z1.z0.conj (z1 - z0) + z1 - z0) / (1 - |z1.z0|^2) *) let
           dz = sub z1 z0 in let z0z1 = mul z0 z1 in let p = sdiv (add (mul
           z0z1 (conj dz)) dz) (1. -. sq_norm z0z1) in tr' := compose !tr
           (p, one); *)
        let p, _ = !tr in
      ...
      (fun x y ->
        ...
        then
          match nodes.(i) with
          | _, `Img (img, name) ->
              ignore (show_image all_messages image_info name img)
          | _ -> () ) ;
    ...
    let rec make_buttons () =
      ...
      info##.onclick :=
        Html.handler (fun _ ->
            ...
            Js._false ) ;
      let tt =
        tooltip (opt_style messages##.info (Js.string "Information"))
      in
      ...
      Dom.appendChild dd
        (doc##createTextNode
           (opt_style messages##.languages (Js.string "Languages"))) ;
      Dom.appendChild dl dd ;
      ...
      recenter##.onclick :=
        Html.handler (fun _ ->
            ...
            Js._false ) ;
      let tt =
        tooltip (opt_style messages##.recenter (Js.string "Recenter"))
      in
      tt##.style##.right := Js.string "36px" ;
      tt##.style##.bottom := Js.string "36px" ;
