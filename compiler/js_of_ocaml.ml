...
let gen_file file f =
  let f_tmp =
    temp_file_name ~temp_dir:(Filename.dirname file)
...
let f
  ...
  let include_dir =
    List.map
      (fun d ->
        ...
        | Some d ->
            let pkg, d' =
              match Util.split Filename.dir_sep d with
              | [] -> assert false
              | [d] -> "js_of_ocaml", d
  ...
  let expunge =
    ...
    | Some file ->
        ...
        close_in ic ;
        Some
          (fun s -> try Hashtbl.find t s ; `Keep with Not_found -> `Skip)
  in
  ...
  let need_debug =
    if source_map <> None || Option.Optim.debuginfo ()
    then `Full
    else if Option.Optim.pretty ()
    then `Names
    else `No
  in
  let p, cmis, d, standalone =
    ...
    then
      ...
      | None ->
          Parse_bytecode.from_channel ~includes:paths ~toplevel ?expunge
            ~dynlink ~debug:need_debug stdin
      | Some f ->
          let ch = open_in_bin f in
          let res =
            Parse_bytecode.from_channel ~includes:paths ~toplevel ?expunge
              ~dynlink ~debug:need_debug ch
          in
          close_in ch ; res
  in
  let () =
    if not runtime_only && source_map <> None
       && Parse_bytecode.Debug.is_empty d
    then
      Util.warn
        "Warning: '--source-map' is enabled but the bytecode program was \
         compiled with no debugging information.\n\
  ...
  | None ->
      ...
      Driver.f ~standalone ?profile ~linkall ~global ~dynlink ?source_map
        ?custom_header fmt d p
  | Some file ->
      gen_file file (fun chan ->
          let p =
            if fs_output = None then PseudoFs.f p cmis fs_files paths else p
          in
      ...
      Util.opt_iter
        (fun file ->
          gen_file file (fun chan ->
              ...
              Driver.f ~standalone ?profile ?custom_header ~global pfs_fmt d
                pfs ) )
        fs_output ) ;
...
let _ =
  ...
  try
    Cmdliner.Term.eval ~catch:false
      ~argv:(Util.normalize_argv ~warn_:true Sys.argv)
      main
  with
  ...
  | Util.MagicNumber.Bad_magic_version h ->
      ...
      let k =
        match Util.MagicNumber.kind h with
        | (`Cmo | `Cma | `Exe) as x -> x
      ...
      in
      Format.eprintf
        "%s: Error: Your ocaml bytecode and the js_of_ocaml compiler have \
         to be compiled with the same version of ocaml.@."
        Sys.argv.(0) ;
      Format.eprintf
        "%s: Error: The Js_of_ocaml compiler has been compiled with ocaml \
         version %s.@."
        Sys.argv.(0) Sys.ocaml_version ;
      Format.eprintf
        "%s: Error: Its seems that your ocaml bytecode has been compiled \
         with %s version of ocaml.@."
        Sys.argv.(0) comp ;
      exit 1
  | Failure s ->
