...
let options =
  ...
  let js_files =
    ...
    in
    let open Arg in
    value & pos_left ~rev:true 0 string [] & info [] ~docv:"JS_FILES" ~doc
  in
  let output_file =
  ...
  let profile =
    ...
    let profile =
      List.map (fun (i, p) -> string_of_int i, p) Driver.profiles
    in
    let open Arg in
    value & opt (some (enum profile)) None & info ["opt"] ~docv:"NUM" ~doc
  in
  let noruntime =
  ...
  let no_sourcemap =
    let doc =
      "Don't generate source map. All other source map related flags will \
       be be ignored."
    in
  ...
  let wrap_with_function =
    let doc =
      "Wrap the generated JavaScript code inside a function that needs to \
       be applied with the global object."
    in
    Arg.(value & opt (some string) None & info ["wrap-with-fun"] ~doc)
  in
  let set_param =
    let doc = "Set compiler options." in
    let all = List.map (fun (x, _) -> x, x) (Option.Param.all ()) in
    let open Arg in
    value
  ...
  let export_file =
    ...
    & info ["export"] ~docs:toplevel_section ~doc
  in
  let linkall =
  ...
  let nocmis =
    ...
    let open Arg in
    value & flag & info ["nocmis"; "no-cmis"] ~docs:toplevel_section ~doc
  in
  let include_dir =
    let doc = "Add [$(docv)] to the list of include directories." in
    let open Arg in
    value & opt_all string []
    & info ["I"] ~docs:filesystem_section ~docv:"DIR" ~doc
  in
  let fs_files =
  ...
  in
  let build_t common set_param set_env dynlink linkall toplevel export_file
      wrap_with_fun include_dir fs_files fs_output fs_external nocmis
      profile noruntime runtime_only no_sourcemap sourcemap
      sourcemap_inline_in_js sourcemap_don't_inline_content sourcemap_root
      output_file input_file js_files =
    ...
    let source_map =
      ...
      then (
        Util.warn
          "Warning: '--source-map' flag ignored because js_of_ocaml was \
           compiled without sourcemap support.\n\
           %!" ;
        None )
      else source_map
    in
  ...
  let t =
    let open Term in
    pure build_t $ CommonArg.t $ set_param $ set_env $ dynlink $ linkall
    $ toplevel $ export_file $ wrap_with_function $ include_dir $ fs_files
    $ fs_output $ fs_external $ nocmis $ profile $ noruntime $ runtime_only
    $ no_sourcemap $ sourcemap $ sourcemap_inline_in_js
    $ sourcemap_don't_inline_content $ sourcemap_root $ output_file
    $ input_file $ js_files
  in
  Term.ret t
  ...
  let man =
    ...
    ; `P
        "Js_of_ocaml is a compiler from OCaml bytecode to Javascript. It \
         makes it possible to run pure OCaml programs in JavaScript \
    ...
    ; `P
        "js_of_ocaml is free software, you can redistribute it and/or \
         ...
         linking exception; either version 2.1 of the License, or (at your \
