...
type 'a on_off = {enable: 'a; disable: 'a}
...
let debug =
  ...
  let arg =
    let open Arg in
    value
    & opt_all (list (enum all)) []
    & info ["debug"] ~docv:"SECTION" ~doc
  in
...
let enable =
  ...
  let arg =
    let open Arg in
    value & opt_all (list (enum all)) [] & info ["enable"] ~docv:"OPT" ~doc
  in
  Term.(pure List.flatten $ arg)
...
let disable =
  ...
  let arg =
    let open Arg in
    value & opt_all (list (enum all)) [] & info ["disable"] ~docv:"OPT" ~doc
  in
  Term.(pure List.flatten $ arg)
...
let t =
  ...
  $ debug $ enable $ disable $ pretty $ debuginfo $ noinline $ quiet
  $ custom_header

let on_off on off t = List.iter on t.enable ; List.iter off t.disable
