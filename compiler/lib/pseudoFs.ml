...
let list_files name paths =
  let name, virtname =
    ...
    | Some i ->
        ...
        then
          failwith
            (Printf.sprintf "path '%s' for file '%s' must be absolute" dest
               src) ;
        let virtname =
  ...
  in
  let file =
    try Util.find_in_findlib_paths paths name with Not_found ->
...
let program_of_files l =
  let fs = List.map (fun (name, filename) -> read name filename) l in
  let body =
    List.map
      (fun (n, c) ->
...
let make_body prim cmis files paths =
  ...
  then (
    ...
    (* [`ocamlc -where`/expunge in.byte out.byte moduleA moduleB ...
       moduleN] *)
  ...
  let body =
    List.map
      (fun (n, c) -> Let (Var.fresh (), Prim (Extern prim, [n; c])))
      fs
  in
  body
