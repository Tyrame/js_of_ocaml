...
let extra_js_files =
  ["+graphics.js"; "+toplevel.js"; "+nat.js"; "+dynlink.js"]
...
module Param = struct
    ...
    let set : string -> unit =
     fun v ->
      try state := convert v with _ ->
        Util.warn "Warning: malformed option %s=%s. IGNORE@." name v
    in
  ...
  (* V8 "optimize" switches with less than 128 case. 60 seams to perform
     well. *)
  let switch_max_case =
    p ~name:"switch_size" ~desc:"set the maximum number of case in a switch"
      (int 60)

  let tailcall_max_depth =
    p ~name:"tc_depth"
      ~desc:
        "set the maximum number of recursive tailcalls defore returning a \
         trampoline"
      (int 50)
  ...
  let _tc_all =
    tc_default :: List.filter (( <> ) tc_default) [TcNone; TcTrampoline]

  let tailcall_optim =
    p ~name:"tc" ~desc:"Set tailcall optimisation"
