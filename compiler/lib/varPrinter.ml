...
let propagate_name t v v' =
  try
    ...
    name_raw t v' name
    (* ; * (try * let n = Hashtbl.find t.names v' in * if n <> name * then
       Printf.eprintf "erasing name %s by %s\n%!" n name * with _ -> ()) *)
  with Not_found -> ()
...
let get_name t v =
  try Some (Hashtbl.find t.names v) with Not_found -> None
...
let rec format_ident x =
  ...
  if x < 54
  then char c1 x
  else format_ident ((x - 54) / 64) ^ char c2 ((x - 54) mod 64)
  ...
  try Hashtbl.find t.cache (i, origin) with Not_found ->
    let name =
      try Hashtbl.find t.known i with Not_found ->
        ...
        if is_reserved s
        then to_string t i
        else ( Hashtbl.add t.known i s ; s )
    in
    let name =
      if t.pretty
      then
