...
let rec enot_rec e =
  let ((_, cost) as res) =
    ...
    | J.EBin (op, e1, e2) -> (
      ...
      (*Disabled: this is not correct! var x = 0/0; !(x < 0) and x >= 0 give
        different result | J.Lt -> (J.EBin (J.Le, e2, e1), 0) | J.Le ->
        (J.EBin (J.Lt, e2, e1), 0) *)
      | _ ->
          J.EUn (J.Not, e), 1 )
    | J.EUn (J.Not, e) -> e, 0
...
let rec if_statement_2 e loc iftrue truestop iffalse falsestop =
  ...
  | _ ->
    ...
    try
      ...
      let exp =
        if e1 = e then J.EBin (J.Or, e, e2) else J.ECond (e, e1, e2)
      in
...
let rec get_variable acc = function
  ...
  | J.EVar (J.S _) -> acc
  | J.EFun _ | J.EStr _ | J.EBool _ | J.ENum _ | J.EQuote _ | J.ERegexp _ ->
      acc
  | J.EArr a ->
      List.fold_left
        (fun acc i ->
          match i with None -> acc | Some e1 -> get_variable acc e1 )
        acc a
  | J.EObj l ->
      List.fold_left (fun acc (_, e1) -> get_variable acc e1) acc l
