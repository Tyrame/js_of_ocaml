    ...
    then
      ...
      then
        ...
        then (
          ...
          for _i = !gen_line to c.gen_line - 1 do Buffer.add_char buf ';'
          done ;
          gen_col := 0 ;
          gen_line := c.gen_line )
        else if i > 0
        ...
        let l =
          ...
          then []
            ...
            :: (c.ori_line - !ori_line) :: (c.ori_col - !ori_col)
            ::
...
let mapping_of_string str =
  ...
  let rec readline line pos acc =
    ...
    then acc
      ...
      let last =
        try String.index_from str pos ';' with Not_found -> total_len
      in
      gen_col := 0 ;
  ...
  | (gen_line_offset, _file, x) :: rest ->
      let rec loop acc ~sources_offset ~names_offset l =
        ...
        | (gen_line_offset, _, sm) :: rest ->
            let acc =
              ...
              ; sources_content=
                  merge_sources_content acc.sources_content
                    sm.sources_content
              ; mappings=
                  acc.mappings
                  @ List.map
      ...
      let acc =
        { x with
          mappings=
            List.map
              (maps ~gen_line_offset ~sources_offset:0 ~names_offset:0)
              x.mappings }
      in
      Some
        (loop acc ~sources_offset:(List.length x.sources)
           ~names_offset:(List.length x.names) rest)

(* let empty = { version = 3; file = "file"; sourceroot = None; sources =
   []; sources_content = None; names = []; mappings = [] }

   let gen (gen_line, gen_col) (ori_line, ori_col) ori_source = { gen_line;
   gen_col; ori_source; ori_line; ori_col; ori_name = None; }

   let s1 = { empty with names = ["na";"nb";"nc"]; sources = ["sa";"sb"];
   mappings = [ gen (1,1) (10,10) 1; gen (3,3) (20,20) 2; ] }

   let s2 = { empty with names = ["na2";"nb2"]; sources = ["sa2"]; mappings
   = [ gen (3,3) (5,5) 1; ] }

   let () = let m = merge [(0,"",s1);(20,"",s2)] in begin match m with |
   None -> () | Some sm -> assert (string_of_mapping sm.mappings =
   ";CCUU;;GCUU;;;;;;;;;;;;;;;;;;;;GCff") end; *)
