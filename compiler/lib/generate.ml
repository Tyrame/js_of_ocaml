 ...
 *)
(*XXX Patterns: => loops should avoid absorbing the whole continuation...
  (detect when the continuation does not loop anymore and close the loop at
  this point) => should have special code for switches that include the
  preceding if statement when possible => if e1 then {if e2 then P else Q}
  else {if e3 then P else Q} => if e then return e1; return e2 => if e then
  var x = e1; else var x = e2; => while (true) {.... if (e) continue; break;
  }
...
let string_of_set s =
  String.concat ", " (List.map string_of_int (AddrSet.elements s))
...
let list_group f g l =
  match l with [] -> [] | a :: r -> list_group_rec f g r (f a) [g a] []
...
module Share = struct
  type 'a aux =
    {strings: 'a StringMap.t; applies: 'a IntMap.t; prims: 'a StringMap.t}
  ...
  let rec get_constant c t =
    ...
    | Tuple (_, args) ->
        Array.fold_left (fun t c -> get_constant c t) t args
    | _ -> t

  let add_args args t =
    List.fold_left
      (fun t a -> match a with Pc c -> get_constant c t | _ -> t)
      t args

  let get ?(alias_strings= false) ?(alias_prims= false) ?(alias_apply= true)
      (_, blocks, _) : t =
    let count =
      AddrMap.fold
        (fun _ block share ->
    ...
    let count =
      List.fold_left
        (fun acc x -> add_special_prim_if_exists x acc)
        count
        [ "caml_trampoline"
  ...
  let get_string gen s t =
    ...
    then gen s
      ...
      try
        ...
        if c > 1
        then (
          try J.EVar (StringMap.find s t.vars.strings) with Not_found ->
...
let rec constant_rec ~ctx x level instrs =
  ...
  | String s ->
      ...
      let p =
        Share.get_prim (runtime_fun ctx) "caml_new_string" ctx.Ctx.share
      in
  ...
  | Tuple (tag, a) -> (
      ...
      | Some elts_rev ->
          ...
          in
          let p =
            Share.get_prim (runtime_fun ctx) "caml_list_of_js_array"
              ctx.Ctx.share
          in
              ...
              List.fold_left
                (fun (acc, instrs) js ->
                  ...
                  | J.EArr _ ->
                      ...
                      let instrs =
                        (J.Variable_statement [J.V v, Some (js, J.N)], J.N)
                        :: instrs
                      in
                      Some (J.EVar (J.V v)) :: acc, instrs
                  | _ -> Some js :: acc, instrs )
                ([], instrs) l
            else List.rev_map (fun x -> Some x) l, instrs
          in
  ...
  try
    ...
    then (elt.prop, elt.ce), List.remove_assoc x queue
      ...
      , List.map
          (function
              | x', elt when x = x' ->
                  x', {elt with cardinal= pred elt.cardinal}
              | x -> x)
          queue )
...
let access_queue' ~ctx queue x =
  ...
  | Pc c ->
      let js, instrs =
        constant ~ctx c (Option.Param.constant_max_depth ())
      in
      assert (instrs = []) ;
...
let access_queue_may_flush queue v x =
  ...
  let _, instrs, queue =
    List.fold_left
      (fun (deps, instrs, queue) ((y, elt) as eq) ->
...
let enqueue expr_queue prop x ce loc cardinal acc =
  let instrs, expr_queue =
    if Option.Optim.compact ()
    then
      if is_mutable prop
      then flush_queue expr_queue prop []
      else [], expr_queue
...
(* This as to be kept in sync with the way we build conditionals and
   switches! *)
...
module DTree = struct
  ...
  let normalize a =
    ...
    >> List.stable_sort (fun (_, l1) (_, l2) ->
  ...
  let build_switch (a: cont array) : 'a t =
    ...
    let rec loop low up =
      let array_norm : (Code.cont * int list) array =
        normalize (Array.sub ai low (up - low + 1))
      in
      ...
      then Branch (fst array_norm.(0))
        ...
        with Not_found ->
          ...
          if !nbcases <= m
          then Switch (Array.map (fun (x, l) -> l, Branch x) array_norm)
          else
            let h = (up + low) / 2 in
            ...
            | [], _ | _, [] -> assert false
            | _, lower_bound2 :: _ ->
                If (Code.CLe (Int32.of_int lower_bound2), b2, b1)
    in
  ...
  let rec fold_cont f b acc =
    ...
    | Switch a ->
        Array.fold_left (fun acc (_, b) -> fold_cont f b acc) acc a
...
let rec build_graph st pc anc =
  if not (AddrSet.mem pc st.visited_blocks)
  then (
  ...
  then
    ...
    then
      ...
      List.fold_right
        ...
        s (visited, grey)
    else visited, if v = 1 then AddrSet.add pc grey else grey
  else visited, grey
...
let generate_apply_fun ctx n =
  ...
  J.EFun
    ...
    , f :: params
    , [ ( J.Statement
            (J.Return_statement (Some (apply_fun_raw ctx f' params')))
        , J.N ) ]
    , J.N )
...
let apply_fun ctx f params loc =
  ...
  then apply_fun_raw ctx f params
    ...
    let y =
      Share.get_apply (generate_apply_fun ctx) (List.length params)
        ctx.Ctx.share
    in
...
let _ =
  List.iter
    (fun (nm, nm') -> Primitive.alias nm nm')
    [ "%int_mul", "caml_mul"
    ; "%int_div", "caml_div"
    ; "%int_mod", "caml_mod"
...
let _ =
  ...
  register_bin_prim "%int_lsr" `Pure (fun cx cy _ ->
  ...
  register_bin_prim "caml_js_get" `Mutable (fun cx cy _ ->
      J.EAccess (cx, cy) ) ;
...
(****)
(* when raising ocaml exception and [improved_stacktrace] is enabled, tag
   the ocaml exception with a Javascript error (that contain js stacktrace).
   {[ throw e ]} becomes {[ throw (caml_exn_with_js_backtrace(e,false)) ]} *)
...
let rec translate_expr ctx queue loc _x e level : _ * J.statement_list =
  ...
  | Closure (args, ((pc, _) as cont)) ->
      ...
      let clo =
        match clo with (st, J.N) :: rem -> (st, J.U) :: rem | _ -> clo
      in
  ...
  | Prim (p, l) ->
      let res =
        ...
        | Extern "caml_js_var", [Pc (String nm | IString nm)]
         |( Extern ("caml_js_expr" | "caml_pure_js_expr")
          , [Pc (String nm | IString nm)] ) -> (
          try
            let offset =
              match loc with J.N | J.U -> None | J.Pi pi -> Some pi
            in
          ...
          with Parse_js.Parsing_error pi ->
            failwith
              (Printf.sprintf "Parsing error %S at l:%d col:%d" nm
                 (pi.Parse_info.line + 1) pi.Parse_info.col) )
        | Extern "%js_array", l ->
            let args, prop, queue =
              List.fold_right
                (fun x (args, prop, queue) ->
        ...
        | Extern "%closure", [Pc (IString name | String name)] ->
            let prim =
              Share.get_prim (runtime_fun ctx) name ctx.Ctx.share
            in
            prim, const_p, queue
        ...
        | ( Extern "%overrideMod"
          , [Pc (String m | IString m); Pc (String f | IString f)] ) ->
        ...
        | Extern "caml_alloc_dummy_function", [_; size] ->
            ...
            let args =
              Array.to_list
                (Array.init (Int32.to_int i) (fun _ -> J.V (Var.fresh ())))
            in
            let f = J.V (Var.fresh ()) in
            let call =
              J.ECall
                ( J.EDot (J.EVar f, "fun")
                , List.map (fun v -> J.EVar v) args
                , loc )
            in
            let e =
              J.EFun
                ( Some f
                , args
                , [J.Statement (J.Return_statement (Some call)), J.N]
                , J.N )
            in
            e, const_p, queue
        ...
        | Extern name, l -> (
            ...
            | None ->
                ...
                then
                  failwith
                    (Printf.sprintf "Unresolved internal primitive: %s" name) ;
                let prim =
                  Share.get_prim (runtime_fun ctx) name ctx.Ctx.share
                in
        ...
        | IsInt, [x] ->
            (* JavaScript engines recognize the pattern 'typeof
               x==="number"'; if the string is shared, less efficient code
               is generated. *)
            let (px, cx), queue = access_queue' ~ctx queue x in
        ...
        | Ult, [x; y] ->
            ...
            , or_p px py
            , queue )
        | ( (Vectlength | Array_get | Not | IsInt | Eq | Neq | Lt | Le | Ult)
          , _ ) ->
            assert false
      in
      res, []
...
and translate_instr ctx expr_queue loc instr =
  ...
  | Let (x, e) -> (
      ...
      | 1, _
        when Option.Optim.compact ()
             && (not (Option.Optim.pretty ()) || not (keep_name x)) ->
          enqueue expr_queue prop x ce loc 1 instrs
      (* We could inline more. size_v : length of the variable after
         serialization size_c : length of the constant after serialization
         num : number of occurrence size_c * n < size_v * n + size_v + 1 +
         size_c *)
...
and compile_block st queue (pc: addr) frontier interm =
  if queue <> [] && (AddrSet.mem pc st.loops || not (Option.Optim.inline ()))
  then flush_all queue (compile_block st [] pc frontier interm)
  else (
    if pc >= 0
    then (
      if AddrSet.mem pc st.visited_blocks
      then (
    ...
    let body =
      ...
      | Code.Pushtrap ((pc1, args1), x, (pc2, args2), pc3s) ->
                ...
                then
                  ...
                  let rec limit pc =
                    ...
                    then false
                      ...
                      | _ -> true
                  in
                  if limit pc3 then AddrSet.add pc3 acc else acc
                else acc )
              pc3s AddrSet.empty
          in
          ...
          let body =
            compile_branch st [] (pc1, args1) None AddrSet.empty
              inner_frontier new_interm
          in
            ...
            then
              ( J.Expression_statement
                  (J.EBin
                     ...
                     , J.EVar (J.V x)
                     , J.ECall
                         ( Share.get_prim (runtime_fun st.ctx)
                             "caml_wrap_exception" st.ctx.Ctx.share
                         , [J.EVar (J.V x)]
                         , J.N ) ))
              , J.N )
              :: handler
      ...
      | _ ->
          ...
          let cond =
            compile_conditional st queue pc block.branch block.handler backs
              new_frontier new_interm succs
          in
          cond
          @
          if AddrSet.cardinal new_frontier = 0
          then []
          else
    ...
    then
      ...
      let st =
        ( J.For_statement
            ...
            , None
            , ( J.Block
                  ( if AddrSet.cardinal frontier > 0
                  then (
                    if debug ()
                    then
...
and colapse_frontier st new_frontier interm =
  if AddrSet.cardinal new_frontier > 1
  then (
    if debug ()
    then
      Format.eprintf "colapse frontier into %d: %s@." st.interm_idx
    ...
    <- AddrMap.add idx
         ...
         st.blocks ;
    (* There is a branch from this switch to the members of the frontier. *)
    AddrSet.iter (fun pc -> incr_preds st pc) new_frontier ;
    (* Put a limit: we are going to remove other branches to the members of
       the frontier (in compile_conditional), but they should remain in the
       frontier. *)
    ...
    , Array.fold_right
        ...
        (Array.mapi (fun i pc -> pc, i) a)
        interm ) )
  else new_frontier, interm

and compile_decision_tree st _queue handler backs frontier interm succs loc
    cx dtree =
  (* Some changes here may require corresponding changes in function
     [DTree.fold_cont] above. *)
  let rec loop cx = function
    | DTree.Empty -> assert false
    | DTree.Branch ((pc, _) as cont) ->
        (* Block of code that never continues (either returns, throws an
           exception or loops back) *)
        (* If not found in successors, this is a backward edge *)
        let never =
          let d =
            try List.assoc pc succs with Not_found -> AddrSet.empty
          in
    ...
    | DTree.If (cond, cont1, cont2) ->
        ...
        in
        ( never1 && never2
        , Js_simpl.if_statement e' loc (J.Block iftrue, J.N) never1
        ...
        let l =
          List.flatten
            (List.map
               (fun (ints, br) ->
                 map_last
                   (fun last i -> J.ENum (float i), if last then br else [])
                   ints )
               l)
        in
...
and compile_conditional st queue pc last handler backs frontier interm succs =
  List.iter
    (fun (pc, _) -> if AddrMap.mem pc interm then decr_preds st pc)
    succs ;
  ( if debug ()
  then
    match last with
  ...
  let res =
    ...
    | Switch (x, a1, [||]) ->
        ...
        let code =
          compile_decision_tree st queue handler backs frontier interm succs
            loc cx (DTree.build_switch a1)
        in
        flush_all queue code
    | Switch (x, a1, a2) ->
        (* The variable x is accessed several times, so we can directly
           refer to it *)
        (* We do not want to share the string "number". See comment for
           IsInt *)
        let b1 =
          compile_decision_tree st queue handler backs frontier interm succs
            loc (var x) (DTree.build_switch a1)
        in
        let b2 =
          compile_decision_tree st queue handler backs frontier interm succs
            loc
        ...
        let code =
          Js_simpl.if_statement
            ...
            loc (Js_simpl.block b1) false (Js_simpl.block b2) false
        in
        flush_all queue code
  in
  ( if debug ()
  ...
  then continuation queue
    ...
    | Some (x0, (h_pc, h_args)) ->
        let old_args =
          ...
          | None -> []
        in
        (* When an extra block is inserted during code generation, args is
           [] *)
        let m =
          Subst.build_mapping (if args = [] then [] else block.params) args
        in
        ...
        let rec loop continuation old args params queue =
          ...
          | x :: args, y :: params ->
              ...
              in
              let x' =
                try Some (VarMap.find x m) with Not_found -> Some x
              in
              ...
              then loop continuation old args params queue
                ...
                let (px, cx), queue = access_queue queue x in
                let st, queue =
                  (*FIX: we should flush only the variables we need rather
                    than doing this; do the same for closure free variables *)
...
and compile_branch st queue ((pc, _) as cont) handler backs frontier interm =
  compile_argument_passing st.ctx queue cont backs (fun queue ->
      compile_exn_handling st.ctx queue cont handler (fun queue ->
          ...
          else if AddrSet.mem pc frontier || AddrMap.mem pc interm
          then (
...
and compile_closure ctx at_toplevel (pc, args) =
  ...
  let res =
    compile_branch st [] (pc, args) None AddrSet.empty AddrSet.empty
      AddrMap.empty
  in
  if AddrSet.cardinal st.visited_blocks <> AddrSet.cardinal current_blocks
  then (
        ...
        (J.Variable_statement
           ( ( match ctx.Ctx.exported_runtime with
             ...
             | Some v ->
                 [ ( J.V v
                   , Some
                       ( J.EDot (s_var Option.global_object, "jsoo_runtime")
                       , J.N ) ) ] )
           @ List.map
               (fun (s, v) -> v, Some (str_js s, J.N))
               (StringMap.bindings ctx.Ctx.share.Share.vars.Share.strings)
           @ List.map
               (fun (s, v) -> v, Some (runtime_fun ctx s, J.N))
