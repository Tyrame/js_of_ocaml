...
let compile b l =
  List.iter
    (fun i ->
      let i = match i with `C i -> i | `I i -> to_int i in
      int_to_buf b i )
    l

let compile_to_string l =
  let b = Buffer.create 50 in
  compile b l ; Buffer.contents b
...
let rec print_obj f x =
  ...
  then
    ...
    then Format.fprintf f "%S" (Obj.magic x : string)
    else if tag = Obj.double_tag
    then Format.fprintf f "%.12g" (Obj.magic x : float)
    ...
    then Format.fprintf f "%ldl" (Obj.magic x : int32)
    else if tag = Obj.custom_tag && same_custom x 0n
    then Format.fprintf f "%ndn" (Obj.magic x : nativeint)
    else if tag = Obj.custom_tag && same_custom x 0L
    then Format.fprintf f "%LdL" (Obj.magic x : int64)
    ...
    then (
      ...
      | n ->
          ...
          for i = 1 to n - 1 do
            Format.fprintf f ", " ;
            print_obj f (Obj.field x i)
          done ;
          Format.fprintf f ")" )
    else Format.fprintf f "<tag %d>" tag
  else Format.fprintf f "%d" (Obj.magic x : int)
