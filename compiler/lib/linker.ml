...
let parse_annot loc s =
  ...
  | Not_found -> None
  | _exc ->
      (* Format.eprintf "Not found for %s : %s @." (Printexc.to_string exc)
         s; *)
      None
...
let parse_file f =
  let file =
    try
      ...
      | Some f ->
          let pkg, f' =
            match Util.split Filename.dir_sep f with
            | [] -> assert false
            | [f] -> "js_of_ocaml-compiler", f
            | pkg :: l -> pkg, List.fold_left Filename.concat "" l
          in
      ...
      (fun (status, lexs) t ->
        ...
        | Js_token.TComment (info, str) -> (
          ...
          | Some a ->
            ...
            | `Code (an, co) ->
                `Annot [a], (List.rev an, List.rev co) :: lexs )
        | _ when Js_token.is_comment t -> status, lexs
        | Js_token.TUnknown (info, _) ->
            Format.eprintf "Unknown token while parsing JavaScript at %s@."
              (loc (Some info)) ;
            if not (Filename.check_suffix file ".js")
            then
              Format.eprintf "%S doesn't look like a JavaScript file@." file ;
      ...
      (fun (annot, code) ->
        ...
        try
          ...
          let req, has_provide, versions, weakdef =
            List.fold_left
              (fun (req, has_provide, versions, weakdef) a ->
                match a with
                | `Provides (pi, name, kind, ka) ->
                    req, Some (pi, name, kind, ka), versions, weakdef
                | `Requires (_, mn) ->
                    mn @ req, has_provide, versions, weakdef
                | `Version (_, l) ->
                    req, has_provide, l :: versions, weakdef
        ...
        with Parse_js.Parsing_error pi ->
          let name =
            match pi with
            | {Parse_info.src= Some x; _} | {Parse_info.name= Some x; _} ->
                x
            | _ -> "??"
          in
          error "cannot parse file %S (orig:%S from l:%d, c:%d)@." f name
...
class check_and_warn name pi =
  object
    ...
    method merge_info from =
      ...
      let diff =
        StringSet.filter
          (fun s -> String.length s <> 0 && s.[0] <> '_')
          diff
      in
      if not (StringSet.is_empty diff)
      then
        Util.warn "WARN unused for primitive %s at %s:@. %s@." name (loc pi)
          (String.concat ", " (StringSet.elements diff)) ;
...
(* exception May_not_return
   ...
   let all_return p = let open Javascript in let rec loop_st = function | []
   -> raise May_not_return | [Return_statement (Some _), _] -> () |
   [Return_statement None, _] -> raise May_not_return |
   [If_statement(_,th,el), _] -> loop_st [th]; (match el with | None ->
   raise May_not_return | Some x -> loop_st [x]) |
   [Do_while_statement(st,_), _] -> loop_st [st] | [While_statement(_,st),
   _] -> loop_st [st] | [For_statement (_,_,_,st), _] -> loop_st [st] |
   [Switch_statement (_,l,def), _] -> List.iter (fun (_,sts) -> loop_st sts)
   l | [Try_statement(b,_,_),_] -> loop_st b | [Throw_statement _, _] -> ()
   | x::xs -> loop_st xs in let rec loop_sources = function | [] -> raise
   May_not_return | [(Statement x, loc)] -> loop_st [(x, loc)] | [_] ->
   raise May_not_return | x::xs -> loop_sources xs in let rec
   loop_all_sources = function | [] -> () | Statement x :: xs ->
   loop_all_sources xs | Function_declaration(_,_,b,_) :: xs -> loop_sources
   b; loop_all_sources xs in try loop_all_sources p; true with
...
let check_primitive name pi code req =
  ...
  let freename =
    List.fold_left
      (fun freename x -> StringSet.remove x freename)
      freename req
  in
  ...
  then
    Util.warn
      "warning: primitive code does not define value with the expected \
       name: %s (%s)@."
      name (loc pi) ;
  if not (StringSet.is_empty freename)
  then (
    Util.warn "warning: free variables in primitive code %S (%s)@." name
      (loc pi) ;
    Util.warn "vars: %s@."
      (String.concat ", " (StringSet.elements freename)) )

(* ; *)
(* return checks disabled *)
(* if false && not (all_return code) *)
(* then Format.eprintf "warning: returns may be missing for primitive code
   %S (%s)@." name (loc pi) *)
...
let add_file f =
  List.iter
    (fun (provide, req, versions, weakdef, (code: Javascript.program)) ->
      let vmatch =
        match versions with [] -> true | l -> List.exists version_match l
      in
      if vmatch
      then (
        incr last_code_id ;
        ...
        | Some (pi, name, kind, ka) ->
            ...
            let rec find = function
              | [] -> None
              | (J.Function_declaration (J.S {J.name= n; _}, l, _, _), _)
                :: _
                when name = n ->
                  Some (List.length l)
              | _ :: rem -> find rem
...
let check_deps () =
  ...
  Hashtbl.iter
    (fun id (code, requires) ->
      ...
      let requires =
        List.fold_right StringSet.add requires StringSet.empty
      in
...
let load_files l = List.iter add_file l ; check_deps ()
...
and resolve_dep_id_rev visited path id =
  ...
  then (
    ...
    then
      error "circular dependency: %s"
        (String.concat ", "
           (List.map (fun id -> fst (Hashtbl.find provided_rev id)) path)) ;
    visited )
  else
    let path = id :: path in
...
let init () =
  List.fold_left
    ...
    {ids= IntSet.empty; codes= []}
    (List.rev !always_included)
