  ...
  AddrMap.iter
    (fun _pc block ->
      List.iter
        ...
        block.body ;
      Util.opt_iter
        (fun (_, cont) -> cont_deps blocks vars deps defs cont)
        block.handler ;
...
let propagate1 deps defs reprs st x =
  ...
  then prev
    ...
    let s =
      VarSet.fold
        (fun x s -> VarSet.add (repr reprs x) s)
        defs.(idx) VarSet.empty
    in
    defs.(idx) <- s ;
    match VarSet.cardinal s with
    | 1 -> replace deps reprs x (VarSet.choose s)
    | 2 -> (
      match VarSet.elements s with
...
let f p =
  ...
  if times ()
  then Format.eprintf "    phi-simpl. 1: %a@." Util.Timer.print t' ;
  let t' = Util.Timer.make () in
  let subst = solver1 vars deps defs in
  if times ()
  then Format.eprintf "    phi-simpl. 2: %a@." Util.Timer.print t' ;
