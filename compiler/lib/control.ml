...
let concat_blocks pc instr params handler args params' instr' last' =
  (* This is only valid because we know that the params only occur in the
     block *)
...
let rec block_simpl pc (preds, entries, blocks) =
  ...
  (* Format.eprintf "RRRRRRRRRRRRRRR %d@." (AddrSet.cardinal (AddrMap.find
     12644 preds)); *)
  let block = AddrMap.find pc blocks in
  match block.branch with
  ...
  | Branch (pc', args) ->
      let block' = AddrMap.find pc' blocks in
      if false
         (*XXX FIX! not (AddrSet.mem pc' entries) && AddrSet.cardinal
           (AddrMap.find pc' preds) = 1 && block'.params = [] &&
      ...
      then
        (*XXX args = [] && is_trivial block'.body block'.branch *)
        ( AddrMap.add pc' (AddrSet.remove pc (AddrMap.find pc' preds)) preds
        , entries
        , AddrMap.add pc
            (concat_blocks pc block.body block.params block.handler args
               block'.params block'.body block'.branch)
  ...
  | Cond (c, x, cont1, cont2) -> (
      ...
      then
        let blocks =
          AddrMap.add pc {block with branch= Branch cont1} blocks
        in
...
let simpl (pc, blocks, free_pc) =
  ...
  let add_pred pc (pc', _) preds =
    ...
    AddrMap.add pc' (AddrSet.add pc (AddrMap.find pc' preds)) preds
  in
  let preds, entries =
    AddrMap.fold
      (fun pc block (preds, entries) ->
        let entries =
          List.fold_left
            (fun entries i ->
              match i with
              | Let (_, Closure (_, (pc, _))) -> AddrSet.add pc entries
              | _ -> entries )
            entries block.body
        in
        let preds =
          ...
          | Switch (_, a1, a2) ->
              let preds =
                Array.fold_left
                  (fun preds cont -> add_pred pc cont preds)
                  preds a1
              in
              let preds =
                Array.fold_left
  ...
  (* Format.eprintf "RRRRRRRRRRRRRRR %d@." (AddrSet.cardinal (AddrMap.find
     12644 preds));*)
  let _, _, blocks =
    traverse blocks pc block_simpl (preds, entries, blocks)
  in
  pc, blocks, free_pc
