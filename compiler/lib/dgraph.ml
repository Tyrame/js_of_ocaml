...
struct
  type t =
    {domain: NSet.t; fold_children: 'a. (N.t -> 'a -> 'a) -> N.t -> 'a -> 'a}
  ...
  module Solver (D : DOMAIN) = struct
    ...
    let rec traverse g visited stack x =
      ...
      then (
        ...
        let visited =
          g.fold_children
            (fun y visited -> traverse g visited stack y)
            x visited
        in
    ...
    let traverse_all g =
      ...
      let visited =
        NSet.fold
          (fun x visited -> traverse g visited stack x)
          g.domain NSet.empty
      in
    ...
    let f g f =
      ...
      let v =
        NSet.fold
          (fun x v -> incr n ; NMap.add x D.bot v)
          g.domain NMap.empty
      in
      (* let t1 = Util.Timer.get t1 in let t2 = Util.Timer.make () in *)
      let w = {set= g.domain; stack= traverse_all g} in
      (* let t2 = Util.Timer.get t2 in let t3 = Util.Timer.make () in *)
      let res = iterate g f v w in
      (* let t3 = Util.Timer.get t3 in Format.eprintf "YYY %.2f %.2f %.2f@."
         t1 t2 t3; Format.eprintf "YYY %d %d (%f)@." !m !n (float !m /.
         float !n); *)
      res
  end
end
...
struct
  ...
  let invert size g =
    ...
    NSet.iter
      (fun x -> g.iter_children (fun y -> add_edge h y x) x)
      g.domain ;
    { domain= g.domain
  ...
  module Solver (D : DOMAIN) = struct
    ...
    let push x st =
      ...
      then ( Stack.push x st.stack ; NSet.remove st.set x )
