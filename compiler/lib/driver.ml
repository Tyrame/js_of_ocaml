...
let eval (p, info) = if Option.Optim.staticeval () then Eval.f info p else p
...
let o1 : 'a -> 'a =
  print >> tailcall >> flow_simple
  >> (* flow simple to keep information for furture tailcall opt *)
     specialize' >> eval >> inline
  >> (* inlining may reveal new tailcall opt *)
     deadcode >> tailcall >> phi >> flow >> specialize' >> eval >> inline
  >> deadcode >> print >> flow >> specialize' >> eval >> inline >> deadcode
  >> phi >> flow >> specialize >> identity
...
let round1 : 'a -> 'a =
  print >> tailcall >> inline
  >> (* inlining may reveal new tailcall opt *)
     deadcode
  >> (* deadcode required before flow simple -> provided by constant *)
     flow_simple
  >> (* flow simple to keep information for furture tailcall opt *)
     specialize' >> eval >> identity

let round2 = flow >> specialize' >> eval >> deadcode >> o1

let o3 =
  loop 10 "tailcall+inline" round1 1 >> loop 10 "flow" round2 1 >> print
           ...
           let ss =
             List.fold_left
               (fun ss (prov, _, _, _, _) ->
                 ...
                 | _ -> ss )
               StringSet.empty (Linker.parse_file file)
           in
           (file, ss) :: acc
         with _ -> acc )
...
let link ~standalone ~linkall ~export_runtime js =
  ...
  then js
    ...
    in
    let js =
      if Option.Optim.genprim () then gen_missing js missing else js
    in
    ...
    let js =
      ...
      then
        ...
        let all =
          List.map (fun name -> PNI name, EVar (S {name; var= None})) all
        in
        ( Statement
            (Expression_statement
               (EBin
...
let pack ~global js =
  ...
  let js =
    ...
    then (
      ...
      if times ()
      then Format.eprintf "    share constant: %a@." Util.Timer.print t1 ;
      js )
    else js
  in
  let js =
    ...
    then (
      ...
      if times ()
      then Format.eprintf "    compact var decl: %a@." Util.Timer.print t2 ;
      js )
    else js
  in
  (* pack *)
  ...
  let js =
    let f =
      J.EFun
        (None, [J.S {J.name= global_object; var= None}], use_strict js, J.U)
    in
    let expr =
      match global with
      | `Function -> f
            ...
            J.ECall
              ( J.EFun
                  ...
                  , [ ( J.Statement
                          (J.Return_statement
                             (Some
                                (J.EVar (J.S {J.name= "this"; var= None}))))
                      , J.N ) ]
                  , J.N )
              , []
              , J.N )
  ...
  let js =
    ...
    then (
      ...
      if times ()
      then Format.eprintf "    shortten vars: %a@." Util.Timer.print t5 ;
      js )
    else js
  in
...
let f ?(standalone= true) ?(global= `Auto) ?(profile= o1) ?(dynlink= false)
    ?(linkall= false) ?source_map ?custom_header formatter d =
  let exported_runtime = not standalone in
  let linkall = linkall || dynlink in
  configure formatter >> profile >> Generate_closure.f >> deadcode'
  >> generate d ~exported_runtime
  >> link ~standalone ~linkall ~export_runtime:dynlink
  >> pack ~global >> coloring >> check_js
