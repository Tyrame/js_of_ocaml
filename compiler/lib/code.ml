...
module Var : sig
  ...
  let get_loc i =
    try Some (Hashtbl.find locations i) with Not_found -> None
...
let print_arg f a =
  match a with Pv x -> Var.print f x | Pc c -> print_constant f c
...
let print_last f l =
  ...
  | Switch (x, a1, a2) ->
      ...
      Array.iteri
        (fun i cont -> Format.fprintf f "int %d -> %a; " i print_cont cont)
        a1 ;
      Array.iteri
        (fun i cont -> Format.fprintf f "tag %d -> %a; " i print_cont cont)
        a2 ;
      Format.fprintf f "}"
...
let prepend (start, blocks, free_pc) body =
  ...
  let blocks =
    AddrMap.add new_start
      {params= []; handler= None; body; branch= Branch (start, [])}
      blocks
  in
...
let fold_children blocks pc f accu =
  ...
  let accu =
    match block.handler with Some (_, (pc, _)) -> f pc accu | None -> accu
  in
...
let invariant (_, blocks, _) =
  ...
  then
    ...
    let check_expr = function
      ...
      | Block (_, _) -> ()
      | Field (_, _) -> ()
      | Closure (l, cont) -> List.iter define l ; check_cont cont
      | Constant _ -> ()
      | Prim (_, _) -> ()
    in
    ...
    let check_last = function
      ...
      | Switch (_x, a1, a2) ->
          Array.iteri (fun _ cont -> check_cont cont) a1 ;
          Array.iteri (fun _ cont -> check_cont cont) a2
    ...
    AddrMap.iter
      (fun _pc block ->
        ...
        Util.opt_iter (fun (_, cont) -> check_cont cont) block.handler ;
