...
let rec mark_var st x =
  ...
  if (st.live).(x) = 1 then List.iter (fun e -> mark_def st e) (st.defs).(x)
...
and mark_expr st e =
  ...
  | Block (_, a) -> Array.iter (fun x -> mark_var st x) a
...
and mark_reachable st pc =
  if not (AddrSet.mem pc st.reachable_blocks)
  then (
...
let ref_count st i =
  match i with Let (x, _) -> (st.live).(Var.idx x) | _ -> 0
...
let annot st pc xi =
  ...
  then "x"
    ...
    | Instr i ->
        ...
        if c > 0
        then Format.sprintf "%d" c
        else if live_instr st i
        then " "
        else "x"
...
let f ((pc, blocks, free_pc) as program) =
  ...
  AddrMap.iter
    (fun _ block ->
      List.iter
        ...
        block.body ;
      Util.opt_iter
        (fun (_, cont) -> add_cont_dep blocks defs cont)
        block.handler ;
      match block.branch with
    ...
    blocks ;
  let st =
    {live; defs; blocks; reachable_blocks= AddrSet.empty; pure_funs}
  in
  mark_reachable st pc ;
  if debug ()
  then print_program (fun pc xi -> annot st pc xi) (pc, blocks, free_pc) ;
      ...
      (fun pc block blocks ->
        ...
        then blocks
          ...
          AddrMap.add pc
            ...
            ; body=
                List.map
                  (fun i -> filter_closure all_blocks st i)
  ...
  in
  if times ()
  then Format.eprintf "  dead code elim.: %a@." Util.Timer.print t ;
