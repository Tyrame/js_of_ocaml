...
let specialize_instr info i rem =
  ...
  | Let
      ...
      , Prim
          ( Extern
              ( ("caml_js_var" | "caml_js_expr" | "caml_pure_js_expr") as
              prim )
          , [y] ) ) ->
      ( match the_string_of info y with
  ...
  | Let (x, Prim (Extern "caml_js_call", [f; o; a])) ->
      ...
      | Some (Block (_, a)) ->
          ...
          Let
            (x, Prim (Extern "%caml_js_opt_call", f :: o :: Array.to_list a))
      | _ -> i )
      :: rem
  | Let (x, Prim (Extern "caml_js_fun_call", [f; a])) ->
      ...
      | Some (Block (_, a)) ->
          let a = Array.map (fun x -> Pv x) a in
          Let
            (x, Prim (Extern "%caml_js_opt_fun_call", f :: Array.to_list a))
      | _ -> i )
      :: rem
  ...
  | Let (x, Prim (Extern "caml_js_from_string", [y])) ->
      ( match the_string_of info y with
      | Some s when Util.is_ascii s -> Let (x, Constant (IString s))
      | _ -> i )
      :: rem
...
let rec specialize_instrs info checks l =
  ...
  | [] -> []
  | i :: r ->
    (* We make bound checking explicit. Then, we can remove duplicated bound
       checks. Also, it appears to be more efficient to inline the array
       access. The bound checking function returns the array, which allows
       to produce more compact code. *)
    match i with
    | Let (x, Prim (Extern "caml_array_get", [y; z]))
     |Let (x, Prim (Extern "caml_array_get_float", [y; z]))
     |Let (x, Prim (Extern "caml_array_get_addr", [y; z])) ->
        let idx =
          match the_int info z with Some idx -> `Cst idx | None -> `Var z
        in
        if List.mem (y, idx) checks
        then
