...
let optimizable blocks pc _ =
  Code.traverse Code.fold_children
    (fun pc acc ->
      if not acc
      then acc
      else
                  ...
                  | Let
                      ...
                      , Prim
                          ( Extern
                              ...
                              | "caml_pure_js_expr" )
                          , _ ) ) ->
                      (* TODO: we should smarter here and look the generated
                         js *)
...
let rec follow_branch_rec seen blocks = function
  | (pc, []) as k -> (
      ...
      try
        ...
        | {body= []; branch= Branch (pc, []); _}
          when not (AddrSet.mem pc seen) ->
      ...
      List.fold_left
        (fun closures i ->
          ...
          | Let (x, Closure (l, cont)) ->
              ...
              (* we can compute this once during the pass as the property
                 won't change with inlining *)
...
let fold_children blocks pc f accu =
  ...
  | Pushtrap (_, _, (pc1, _), pcs) -> f pc1 (AddrSet.fold f pcs accu)
  | Cond (_, _, (pc1, _), (pc2, _)) -> accu >> f pc1 >> f pc2
  | Switch (_, a1, a2) ->
      accu
      >> Array.fold_right (fun (pc, _) accu -> f pc accu) a1
      >> Array.fold_right (fun (pc, _) accu -> f pc accu) a2

let rewrite_closure blocks cont_pc clos_pc handler =
  Code.traverse fold_children
    (rewrite_block (cont_pc, handler))
    clos_pc blocks blocks

(****)
(* get new location put continuation at new location update closure body to
   return to this location make current block continuation jump to closure
   body *)
...
let simple blocks cont mapping =
  ...
  let rec follow (pc, args) (instr: [`Empty | `Ok of 'a]) mapping =
    ...
    | `Ok (x, exp), Return ret
      ...
      | Apply (f, args, true) ->
          `Exp
            (Apply (map_var mapping f, List.map (map_var mapping) args, true))
      | Prim (prim, args) ->
  ...
  let body, (branch, blocks, free_pc) =
    List.fold_right
      (fun i (rem, state) ->
        ...
        | Let (x, Apply (f, args, true)) when VarMap.mem f closures -> (
            ...
            | `Fail ->
                if live_vars.(Var.idx f) = 1
                   && outer_optimizable = f_optimizable
                   (* inlining the code of an optimizable function could
                      make this code unoptimized. (wrt to Jit compilers) At
                      the moment, V8 doesn't optimize function containing
                      try..catch. We disable inlining if the inner and outer
                      functions don't have the same "contain_try_catch"
                      property *)
                then
                  let blocks, cont_pc =
                    match rem, branch with
                    | [], Return y when Var.compare x y = 0 ->
                        (* We do not need a continuation block for tail
                           calls *)
                        blocks, None
                    | _ ->
                        ( AddrMap.add free_pc
                  ...
                  in
                  let blocks =
                    rewrite_closure blocks cont_pc (fst clos_cont)
                      block.handler
                  in
                  (* We do not really need this intermediate block. It just
                     avoid the need to find which function parameters are
                  ...
                  ( (* Format.eprintf "Do not inline because inner:%b
                       outer:%b@." f_has_handler outer_has_handler; *)
                    i :: rem
                  , state ) )
        | Let (x, Closure (l, (pc, []))) -> (
            let block = AddrMap.find pc blocks in
            ...
            | { body= [Let (y, Prim (Extern prim, args))]
              ...
              ; params= [] } ->
                ...
                if Code.Var.compare y y' = 0
                   ...
                   && args_equal l args
                then
                  ( Let (x, Prim (Extern "%closure", [Pc (IString prim)]))
                    :: rem
                  , state )
                else i :: rem, state
            | _ -> i :: rem, state )
        | _ -> i :: rem, state )
      block.body
      ([], (block.branch, blocks, free_pc))
  in
  AddrMap.add pc {block with body; branch} blocks, free_pc
    ...
    Code.fold_closures p
      (fun name _ (pc, _) (blocks, free_pc) ->
        let outer_optimizable =
          ...
          | Some x ->
              let _, _, b = VarMap.find x closures in
              b
        in
        Code.traverse Code.fold_children
          (inline closures live_vars outer_optimizable)
          pc blocks (blocks, free_pc) )
      (blocks, free_pc)
  in
