...
let add_assign_def vars defs x y =
  ...
  | Phi s -> defs.(idx) <- Phi (VarSet.add y s)
...
let expr_deps blocks vars deps defs x e =
  ...
  | Block (_, a) -> Array.iter (fun y -> add_dep deps x y) a
...
let propagate1 deps defs st x =
  ...
  | Param -> VarSet.singleton x
  | Phi s -> var_set_lift (fun y -> VarTbl.get st y) s
  | Expr e ->
    match e with
        ...
        var_set_lift
          (fun z ->
            ...
            | Expr (Block (_, a)) when n < Array.length a ->
                let t = a.(n) in
                add_dep deps x t ; VarTbl.get st t
            | Phi _ | Param | Expr _ -> VarSet.empty )
          (VarTbl.get st y)
...
let expr_escape st _x e =
  ...
  | Apply (_, l, _) -> List.iter (fun x -> block_escape st x) l
  | Prim
              ...
              | Expr (Block (_, a)) ->
                  Array.iter
                    (fun x ->
                      ...
                      | Expr (Block (_, [|_k; v|])) -> block_escape st v
                      | _ -> block_escape st x )
                    a
              | _ -> block_escape st v )
...
let propagate2 ?(skip_param= false) defs known_origins possibly_mutable st x =
  match defs.(Var.idx x) with
  | Param -> skip_param
  | Phi s -> VarSet.exists (fun y -> VarTbl.get st y) s
  | Expr e ->
    match e with
...
let solver2 ?skip_param vars deps defs known_origins possibly_mutable =
  ...
  in
  Solver2.f () g
    (propagate2 ?skip_param defs known_origins possibly_mutable)

let get_approx {info_defs= _; info_known_origins; info_maybe_unknown; _} f
    top join x =
  let s = VarTbl.get info_known_origins x in
  if VarTbl.get info_maybe_unknown x
  then top
  else
  ...
  | Pv x ->
      get_approx info
        (fun x ->
          ...
          | Expr (Constant (String _) as e) when Option.Optim.safe_string () ->
              Some e
          | Expr e ->
  ...
  | Pv x ->
      get_approx info
        (fun x ->
          ...
          | Expr (Constant (String _ as c)) when Option.Optim.safe_string () ->
              Some c
          | Expr (Constant c) ->
              if (info.info_possibly_mutable).(Var.idx x)
              then None
        ...
        None
        (fun u v ->
          match u, v with Some i, Some j when i = j -> u | _ -> None )
        x
  | Pc c -> Some c

let the_int info x =
  match the_const_of info x with Some (Int i) -> Some i | _ -> None
      ...
      get_approx info
        (fun z ->
          ...
          then None
            ...
            | Expr (Block (_, a)) when n < Array.length a -> Some a.(n)
            | _ -> None )
        None
        (fun u v ->
          match u, v with
  ...
  VarISet.iter
    (fun x ->
      ...
      then
        ...
        if VarSet.cardinal s = 1
        then subst.(Var.idx x) <- Some (VarSet.choose s) ) ;
      if subst.(Var.idx x) = None
      then subst.(Var.idx x) <- direct_approx info x ;
...
let f ?skip_param p =
  ...
  if times ()
  then Format.eprintf "    flow analysis 1: %a@." Util.Timer.print t1 ;
  let t2 = Util.Timer.make () in
  let known_origins = solver1 vars deps defs in
  if times ()
  then Format.eprintf "    flow analysis 2: %a@." Util.Timer.print t2 ;
  let t3 = Util.Timer.make () in
  let possibly_mutable = program_escape defs known_origins p in
  if times ()
  then Format.eprintf "    flow analysis 3: %a@." Util.Timer.print t3 ;
  let t4 = Util.Timer.make () in
  let maybe_unknown =
    solver2 ?skip_param vars deps defs known_origins possibly_mutable
  in
  if times ()
  then Format.eprintf "    flow analysis 4: %a@." Util.Timer.print t4 ;
  if debug ()
  then
    VarISet.iter
      (fun x ->
        let s = VarTbl.get known_origins x in
        if not (VarSet.is_empty s) (*&& VarSet.choose s <> x*)
        then
          Format.eprintf "%a: {%a} / %s@." Var.print x Code.print_var_list
            (VarSet.elements s)
  ...
  if times ()
  then Format.eprintf "    flow analysis 5: %a@." Util.Timer.print t5 ;
