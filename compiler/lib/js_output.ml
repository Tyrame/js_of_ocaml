 ...
 *)
(* XXX Beware automatic semi-colon insertion... a=b ++c is not the same as
   a=b ++c ===> see so-called "restricted productions": the space cannot be
   replaced by a newline in the following expressions: e ++, e --, continue
   e, break e, return e, throw e *)
...
struct
  ...
  let opt_identifier f i =
    match i with None -> () | Some i -> PP.space f ; ident f i
  ...
  let rec formal_parameter_list f l =
    ...
    | [i] -> ident f i
    | i :: r ->
        ident f i ; PP.string f "," ; PP.break f ; formal_parameter_list f r

  (* 0 Expression 1 AssignementExpression 2 ConditionalExpression 3
     LogicalORExpression 4 LogicalANDExpression 5 BitwiseORExpression 6
     BitwiseXORExpression 7 BitwiseANDExpression 8 EqualityExpression 9
     RelationalExpression 10 ShiftExpression 11 AdditiveExpression 12
     MultiplicativeExpression 13 UnaryExpression 14 PostfixExpression 15
     LeftHandsideExpression NewExpression CallExpression 16 MemberExpression
     FunctionExpression PrimaryExpression *)

  let op_prec op =
    match op with
    | Eq | StarEq | SlashEq | ModEq | PlusEq | MinusEq | LslEq | AsrEq
     |LsrEq | BandEq | BxorEq | BorEq ->
        1, 13, 1
    (* | Or -> 3, 3, 4 | And -> 4, 4, 5 | Bor -> 5, 5, 6 | Bxor -> 6, 6, 7 |
       Band -> 7, 7, 8 *)
    | Or ->
        3, 3, 3
    | And -> 4, 4, 4
  ...
  let rec need_paren l e =
    ...
    | EVar _ | EStr _ | EArr _ | EBool _ | ENum _ | EQuote _ | ERegexp _
     |EUn _ | ENew _ ->
        false
    | EFun _ | EObj _ -> true
  ...
  let pp_string f ?(quote= '"') ?(utf= false) s =
    ...
    for i = 0 to l - 1 do
      ...
      | '\t' -> PP.string f "\\t"
      | '\n' -> PP.string f "\\n"
      (* This escape sequence is not supported by IE < 9 | '\011' -> "\\v" *)
      | '\012' ->
          PP.string f "\\f"
  ...
  and opt_expression l f e =
    match e with None -> () | Some e -> expression l f e
  ...
  and statement ?(last= false) f (s, loc) =
    ...
    | Debugger_statement -> PP.string f "debugger" ; last_semi ()
    | Expression_statement (EVar _) -> last_semi ()
    | Expression_statement e ->
        (* Parentheses are required when the expression starts syntactically
           with "{" or "function" *)
        if need_paren 0 e
        then (
          PP.start_group f 1 ;
    ...
    | ForIn_statement (e1, e2, s) ->
        ...
        PP.end_group f ;
        PP.end_group f
    | Continue_statement None -> PP.string f "continue" ; last_semi ()
    | Continue_statement (Some s) ->
        PP.string f "continue " ;
        PP.string f (Javascript.Label.to_string s) ;
        last_semi ()
    | Break_statement None -> PP.string f "break" ; last_semi ()
    ...
    | Return_statement e -> (
      match e with
      | None -> PP.string f "return" ; last_semi ()
      | Some (EFun (i, l, b, pc)) ->
          PP.start_group f 1 ;
          PP.start_group f 0 ;
          PP.start_group f 0 ;
      ...
      | Some e ->
          ...
          PP.end_group f ;
          PP.end_group f
          (* There MUST be a space between the return and its argument. A
             line return will not work *) )
    ...
    | Throw_statement e ->
        ...
        PP.end_group f ;
        PP.end_group f
    (* There must be a space between the return and its argument. A line
...
let need_space a b =
  ...
  (* do not generate end_of_line_comment. handle the case of "num / /*
     comment */ b " *)
  || (a = '/' && b = '/')
...
let program f ?source_map p =
  let smo =
    match source_map with None -> None | Some (_, sm) -> Some sm
  in
  ...
  | Some (out_file, sm) ->
      ...
      let sources =
        ...
        | Some root ->
            ...
            let targets =
              List.map (fun src -> Filename.basename src) sources
            in
            printf "#! /bin/bash\n" ;
            printf "mkdir -p %s\n" root ;
            List.iter2
            ...
            close_out oc ;
            Util.warn
              "Source-map info: run 'sh %s' to create links to sources in \
               %s.\n\
               %!"
              script_file root ;
            targets
