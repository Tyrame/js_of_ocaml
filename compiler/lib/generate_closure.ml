...
let add_multi k v map =
  let set = try VarMap.find k map with Not_found -> AddrSet.empty in
...
let rec tailcall pc blocks visited tc =
  ...
  then visited, tc
    ...
    let tc_opt =
      ...
      | Return x -> (
        match Util.last block.body with
...
let group_closures closures =
  let names =
    List.fold_left
      (fun names x -> VarSet.add x.f_name names)
      VarSet.empty closures
  in
  let closures_map =
    List.fold_left
      (fun closures_map x -> VarMap.add x.f_name x closures_map)
      VarMap.empty closures
  in
  let graph =
    List.fold_left
      (fun graph x ->
        let tc =
          VarMap.fold (fun x _ tc -> VarSet.add x tc) x.tc VarSet.empty
        in
        let tc = VarSet.inter names tc in
        VarMap.add x.f_name tc graph )
      VarMap.empty closures
  in
  ...
  let f free_pc blocks closures_map component =
    ...
    | SCC.Has_loop all ->
        ...
        then (
          ...
          Format.eprintf "%s\n%!"
            (String.concat ", " (List.map (fun x -> Var.to_string x) all)) ) ;
        let all =
          List.map
            (fun id ->
              Code.Var.fresh_n "counter", VarMap.find id closures_map )
            all
        in
        let blocks, free_pc, instrs, instrs_wrapper =
          List.fold_left
            (fun (blocks, free_pc, instrs, instrs_wrapper) (counter, ci) ->
              if debug_tc ()
              then
              ...
              let instr_wrapper =
                Let (ci.f_name, wrapper_closure wrapper_pc new_args)
              in
              let instr_real =
                Let (new_f, Closure (counter :: ci.args, ci.cont))
              in
              let counter_and_pc =
                List.fold_left
                  (fun acc (counter, ci2) ->
                    try
                      let pcs =
                        AddrSet.elements (VarMap.find ci.f_name ci2.tc)
                      in
                    ...
                    | Let (x, Apply (f, args, true)) :: rem_rev ->
                        ...
                        let blocks =
                          AddrMap.add direct_call_pc
                            (direct_call_block block ~counter ~x ~f:new_f
                               ~args)
                            blocks
                        in
                        let blocks =
                            ...
                            , Prim
                                ...
                                , [ Pv counter
                                  ; Pc
                                      (Int
                                         (Int32.of_int
                                            (Option.Param.tailcall_max_depth
                                               ()))) ] ) )
                        in
                        let block =
                          { block with
                            body= List.rev (last :: rem_rev); branch }
                        in
  ...
  then free_pc, blocks, internal_and_external
    ...
    | _ ->
        ...
        let pcs =
          List.map
            (function
                | Let (_, Closure (_, (pc, _))) -> pc | _ -> assert false)
            internal_and_external
        in
        let old_xs =
          List.map
            (function Let (x, Closure _) -> x | _ -> assert false)
            closures_extern
        in
            ...
            List.map2
              (fun cl new_x ->
                ...
                | Let (_, Closure (params, (pc, pc_args))) ->
                    Let
                      ( new_x
                      , Closure (params, (pc, List.map mapping pc_args)) )
                | _ -> assert false )
...
let rec rewrite_closures mutated_vars rewrite_list free_pc blocks body :
    int * _ * _ list =
  match body with
  | Let (_, Closure _) :: _ ->
      ...
      let free_pc, blocks, closures =
        List.fold_left
          (fun (free_pc, blocks, acc) component ->
            let free_pc, blocks, closures =
              rewrite_tc free_pc blocks closures_map component
            in
            let free_pc, blocks, intrs =
              rewrite_mutable free_pc blocks mutated_vars rewrite_list
                closures
            in
...
let f ((pc, blocks, free_pc) as p) : Code.program =
    ...
    AddrMap.fold
      (fun pc _ (blocks, free_pc) ->
        ...
        let free_pc, blocks, body =
          rewrite_closures mutated_vars rewrite_list free_pc blocks
            block.body
        in
  ...
  let p =
    List.fold_left
      (fun program (mapping, pc) -> Jsoo_subst.cont mapping pc program)
