...
(* let render l = let st = { indent = 0; box_indent = 0; prev_indents = [];
   ...
   stdout s i l } in push st (Start_group 0); List.iter (fun e -> push st e)
   l; push st End_group; output_newline st

   let rec tree n = if n = 0 then [Text "Leaf"] else [Start_group 10; Text
   "Node.... ("] @ tree (n - 1) @ [Text ","; Break (" ", 0)] @ tree (n - 1)
   @ [Text ")"; End_group]

   let _ = for i = 1 to 10 do render (tree i) done *)
...
let output_substring = Pervasives.(output_substring)
