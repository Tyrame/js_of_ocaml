...
module IdentTable = struct
  ...
  and 'a data = {ident: Ident.t; data: 'a; previous: 'a data option}
  ...
  let rec table_contents_rec sz t rem =
    ...
    | Node (l, v, r, _) ->
        table_contents_rec sz l
          ( (sz - v.data, Ident.name v.ident, v.ident)
          :: table_contents_rec sz r rem )
...
type 'a numtable = {num_cnt: int; num_tbl: ('a, int) Tbl.t}
...
module Debug : sig
  ...
  val find :
    data -> Code.addr -> (int * string * Ident.t) list * Env.summary
  ...
  val fold :
    data -> (Code.addr -> Instruct.debug_event -> 'a -> 'a) -> 'a -> 'a
end = struct
  open Instruct
  ...
  type data =
    (int, debug_event * ml_unit) Hashtbl.t
  ...
  let find_ml_in_paths paths name =
    ...
    try Some (Util.find_in_path paths (uname ^ ".ml")) with Not_found ->
      try Some (Util.find_in_path paths (name ^ ".ml")) with Not_found ->
        None
  ...
  let read_event_list =
    let read_paths : in_channel -> string list =
      ...
      | `V4_02 | `V4_03 | `V4_04 | `V4_06 ->
          fun ic -> (input_value ic : string list)
    in
    fun (events_by_pc, units) ~crcs ~includes ~orig ic ->
      ...
      let paths =
        try Some (read_paths ic @ includes) with Failure _ ->
          (* restore position *)
          seek_in ic pos ; None
      in
      match paths with
      ...
      | Some (paths: string list) ->
          List.iter
            (fun ( { ev_module
                   ; ev_loc= {Location.loc_start= {Lexing.pos_fname; _}; _}; _
                   } as ev ) ->
              let unit =
                try Hashtbl.find units (ev_module, pos_fname)
                with Not_found ->
                  let crc =
                    try Hashtbl.find crcs ev_module with Not_found -> None
                  in
                  let source =
  ...
  let fold ((events_by_pc, _): data) f acc =
    Hashtbl.fold (fun k (e, _u) acc -> f k e acc) events_by_pc acc
end
...
module Blocks : sig
  ...
  let rec scan debug blocks code pc len =
    ...
    then
      ...
      | KNullaryCall ->
          let blocks =
            if Debug.mem debug (pc + 1)
            then AddrSet.add pc blocks
            else blocks
          in
          scan debug blocks code (pc + 1) len
      | KUnaryCall ->
          let blocks =
            if Debug.mem debug (pc + 2)
            then AddrSet.add pc blocks
            else blocks
          in
      ...
      | KSwitch ->
          ...
          scan debug !blocks code
            (pc + 2 + (sz land 0xffff) + (sz lsr 16))
            len
      | KClosurerec ->
...
module Constants : sig
  ...
  let warn_overflow i i32 =
    Util.warn
      "Warning: integer overflow: integer %s truncated to 0x%lx (%ld); the \
  ...
  let rec parse x =
    ...
    then
      ...
      then
        Tuple (tag, Array.init (Obj.size x) (fun i -> parse (Obj.field x i)))
      else assert false
    else
...
type globals =
  ...
  ; mutable override:
      (Var.t -> Code.instr list -> Var.t * Code.instr list) option array
...
module State = struct
  ...
  type handler = {var: Var.t; addr: addr; stack_len: int; block_pc: addr}
  ...
  let size_globals st size =
    if size > Array.length st.globals.vars
    then resize_globals st.globals size
  ...
  let start_function state env offset =
    { state with
      accu= Dummy; stack= []; env; env_offset= offset; handlers= [] }
  ...
  let print st =
    Format.eprintf "{ %a | %a | (%d) %a }@." print_elt st.accu print_stack
      st.stack st.env_offset print_env st.env
  ...
  let rec name_rec debug i l s summary =
    ...
    | (j, nm, ident) :: lrem, Var v :: srem when i = j ->
        ( match Util.find_loc_in_summary ident summary with
        | None -> ()
...
let get_global state instrs i =
  ...
  | Some x ->
      if debug_parser ()
      then Format.printf "(global access %a)@." Var.print x ;
  ...
  | None ->
      ...
      then
        ...
        let x, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = CONST(%d)@." Var.print x i ;
        (g.vars).(i) <- Some x ;
        x, state, instrs )
...
type compile_info =
  {blocks: Blocks.u; code: string; limit: int; debug: Debug.data}

let rec compile_block blocks debug code pc state =
  if not (AddrSet.mem pc !tagged_blocks)
  then (
    ...
    let string_of_addr addr =
      ...
      | Some loc ->
        ...
        | None -> string_of_int addr
        | Some file ->
            Printf.sprintf "%s:%d:%d-%d" file loc.Parse_info.line
              loc.Parse_info.col (addr + 2)
    in
    if debug_parser ()
    then
      Format.eprintf "Compiling from %s to %d@." (string_of_addr pc)
        (limit - 1) ;
    let state = State.start_block pc state in
    tagged_blocks := AddrSet.add pc !tagged_blocks ;
    let instr, last, state' =
      compile {blocks; code; limit; debug} pc state []
    in
    ...
    | Switch (_, l1, l2) ->
        Array.iter
          (fun (pc', _) -> compile_block blocks debug code pc' state')
          l1 ;
        Array.iter
          (fun (pc', _) -> compile_block blocks debug code pc' state')
          l2
...
and compile infos pc state instrs =
  ...
  then
    ...
    then (
      ...
      if debug_parser ()
      then Format.eprintf "Branch %d (%a) @." pc print_var_list stack ;
    ...
    let instr =
      try get_instr code pc with Bad_instruction op ->
        if debug_parser () then Format.eprintf "%08x@." op ;
        assert false
    in
    if debug_parser ()
    then Format.eprintf "%08x %s@." instr.opcode instr.name ;
    ...
    | ASSIGN ->
        ...
        (* We switch to a different block as this may have changed the
    ...
    | APPLY1 ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = %a(%a)@." Var.print x Var.print f Var.print y ;
    ...
    | APPLY2 ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = %a(%a, %a)@." Var.print x Var.print f
            Var.print y Var.print z ;
    ...
    | APPLY3 ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = %a(%a, %a, %a)@." Var.print x Var.print f
            Var.print y Var.print z Var.print t ;
    ...
    | APPTERM1 ->
        ...
        if debug_parser ()
        then Format.printf "return %a(%a)@." Var.print f Var.print x ;
    ...
    | APPTERM2 ->
        ...
        if debug_parser ()
        then
          Format.printf "return %a(%a, %a)@." Var.print f Var.print x
            Var.print y ;
    ...
    | APPTERM3 ->
        ...
        if debug_parser ()
        then
          Format.printf "return %a(%a, %a, %a)@." Var.print f Var.print x
            Var.print y Var.print z ;
    ...
    | CLOSURE ->
        ...
        let x, state = State.fresh_var state in
        let env =
          Array.of_list (State.Dummy :: List.map (fun x -> State.Var x) vals)
        in
    ...
    | GETGLOBALFIELD ->
        ...
        let y, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = %a[%d]@." Var.print y Var.print x j ;
    ...
    | SETGLOBAL ->
        ...
        if debug_parser ()
        then Format.printf "(global %d) = %a@." i Var.print y ;
        let instrs =
    ...
    | ATOM ->
        ...
        let x, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = ATOM(%d)@." Var.print x i ;
    ...
    | MAKEBLOCK2 ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = { 0 = %a; 1 = %a; }@." Var.print x Var.print y
            Var.print z ;
    ...
    | MAKEBLOCK3 ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = { 0 = %a; 1 = %a; 2 = %a }@." Var.print x
            Var.print y Var.print z Var.print t ;
    ...
    | GETFIELD0 ->
        ...
        let x, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = %a[0]@." Var.print x Var.print y ;
        compile infos (pc + 1) state (Let (x, Field (y, 0)) :: instrs)
    | GETFIELD1 ->
        let y = State.accu state in
        let x, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = %a[1]@." Var.print x Var.print y ;
        compile infos (pc + 1) state (Let (x, Field (y, 1)) :: instrs)
    | GETFIELD2 ->
        let y = State.accu state in
        let x, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = %a[2]@." Var.print x Var.print y ;
        compile infos (pc + 1) state (Let (x, Field (y, 2)) :: instrs)
    | GETFIELD3 ->
        let y = State.accu state in
        let x, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = %a[3]@." Var.print x Var.print y ;
    ...
    | GETFIELD ->
        ...
        let x, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = %a[%d]@." Var.print x Var.print y n ;
    ...
    | SETFIELD0 ->
        ...
        let z = State.peek 0 state in
        if debug_parser ()
        then Format.printf "%a[0] = %a@." Var.print y Var.print z ;
        let x, state = State.fresh_var state in
        if debug_parser () then Format.printf "%a = 0@." Var.print x ;
        compile infos (pc + 1) (State.pop 1 state)
    ...
    | SETFIELD1 ->
        ...
        let z = State.peek 0 state in
        if debug_parser ()
        then Format.printf "%a[1] = %a@." Var.print y Var.print z ;
        let x, state = State.fresh_var state in
        if debug_parser () then Format.printf "%a = 0@." Var.print x ;
        compile infos (pc + 1) (State.pop 1 state)
    ...
    | SETFIELD2 ->
        ...
        let z = State.peek 0 state in
        if debug_parser ()
        then Format.printf "%a[2] = %a@." Var.print y Var.print z ;
        let x, state = State.fresh_var state in
        if debug_parser () then Format.printf "%a = 0@." Var.print x ;
        compile infos (pc + 1) (State.pop 1 state)
    ...
    | SETFIELD3 ->
        ...
        let z = State.peek 0 state in
        if debug_parser ()
        then Format.printf "%a[3] = %a@." Var.print y Var.print z ;
        let x, state = State.fresh_var state in
        if debug_parser () then Format.printf "%a = 0@." Var.print x ;
        compile infos (pc + 1) (State.pop 1 state)
    ...
    | SETFIELD ->
        ...
        let n = getu code (pc + 1) in
        if debug_parser ()
        then Format.printf "%a[%d] = %a@." Var.print y n Var.print z ;
        let x, state = State.fresh_var state in
        if debug_parser () then Format.printf "%a = 0@." Var.print x ;
        compile infos (pc + 2) (State.pop 1 state)
    ...
    | VECTLENGTH ->
        ...
        let x, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = %a.length@." Var.print x Var.print y ;
    ...
    | SETVECTITEM ->
        if debug_parser ()
        then
          Format.printf "%a[%a] = %a@." Var.print (State.accu state)
            Var.print (State.peek 0 state) Var.print (State.peek 1 state) ;
        let instrs =
          Array_set
            (State.accu state, State.peek 0 state, State.peek 1 state)
          :: instrs
        in
        let x, state = State.fresh_var state in
        if debug_parser () then Format.printf "%a = 0@." Var.print x ;
    ...
    | BRANCHIF ->
        ...
        let args = State.stack_vars state in
        ( instrs
        , Cond (IsTrue, x, (pc + offset + 1, args), (pc + 2, args))
        , state )
    | BRANCHIFNOT ->
        let offset = gets code (pc + 1) in
        let x = State.accu state in
        let args = State.stack_vars state in
        ( instrs
        , Cond (IsTrue, x, (pc + 2, args), (pc + offset + 1, args))
        , state )
    | SWITCH ->
        ...
        let it =
          Array.init (sz land 0XFFFF) (fun i ->
              pc + 2 + gets code (pc + 2 + i), args )
        in
        let bt =
          Array.init (sz lsr 16) (fun i ->
              pc + 2 + gets code (pc + 2 + l + i), args )
        in
    ...
    | BOOLNOT ->
        ...
        let x, state = State.fresh_var state in
        if debug_parser ()
        then Format.printf "%a = !%a@." Var.print x Var.print y ;
    ...
    | POPTRAP ->
        ...
        compile_block infos.blocks infos.debug code addr
          (State.pop 4 (State.pop_handler state)) ;
        ( instrs
        , Poptrap ((addr, State.stack_vars state), handler_addr)
        , state )
    | RERAISE | RAISE_NOTRACE | RAISE ->
        ...
        in
        if debug_parser ()
        then Format.printf "throw(%a)@." Var.print (State.accu state) ;
    ...
    | C_CALL1 ->
        ...
        then (* This is a no-op *)
          ...
          if debug_parser ()
          then
            Format.printf "%a = ccall \"%s\" (%a)@." Var.print x prim
              Var.print y ;
          compile infos (pc + 2) state
    ...
    | C_CALL2 ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = ccall \"%s\" (%a, %a)@." Var.print x prim
            Var.print y Var.print z ;
        compile infos (pc + 2) (State.pop 1 state)
    ...
    | C_CALL3 ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = ccall \"%s\" (%a, %a, %a)@." Var.print x prim
            Var.print y Var.print z Var.print t ;
        compile infos (pc + 2) (State.pop 2 state)
    ...
    | C_CALL4 ->
        ...
        then (
          ...
          Format.printf ")@." ) ;
        compile infos (pc + 2) state
          ( Let (x, Prim (Extern prim, List.map (fun x -> Pv x) args))
          :: instrs )
    | C_CALL5 ->
        let nargs = 5 in
        let prim = primitive_name state (getu code (pc + 1)) in
    ...
    | MODINT ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = %a %% %a@." Var.print x Var.print y Var.print
            z ;
        compile infos (pc + 1) (State.pop 1 state)
    ...
    | EQ ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = mk_bool(%a == %a)@." Var.print x Var.print y
            Var.print z ;
        compile infos (pc + 1) (State.pop 1 state)
    ...
    | BEQ ->
        ...
        let args = State.stack_vars state in
        ( instrs
        , Cond (CEq n, x, (pc + offset + 2, args), (pc + 3, args))
        , state )
    | BNEQ ->
        let n = gets32 code (pc + 1) in
        let offset = gets code (pc + 2) in
        ...
        let args = State.stack_vars state in
        ( instrs
        , Cond (CEq n, x, (pc + 3, args), (pc + offset + 2, args))
        , state )
    | BLTINT ->
        let n = gets32 code (pc + 1) in
        let offset = gets code (pc + 2) in
        ...
        let args = State.stack_vars state in
        ( instrs
        , Cond (CLt n, x, (pc + offset + 2, args), (pc + 3, args))
        , state )
    | BLEINT ->
        let n = gets32 code (pc + 1) in
        let offset = gets code (pc + 2) in
        ...
        let args = State.stack_vars state in
        ( instrs
        , Cond (CLe n, x, (pc + offset + 2, args), (pc + 3, args))
        , state )
    | BGTINT ->
        let n = gets32 code (pc + 1) in
        let offset = gets code (pc + 2) in
        ...
        let args = State.stack_vars state in
        ( instrs
        , Cond (CLe n, x, (pc + 3, args), (pc + offset + 2, args))
        , state )
    | BGEINT ->
        let n = gets32 code (pc + 1) in
        let offset = gets code (pc + 2) in
        ...
        let args = State.stack_vars state in
        ( instrs
        , Cond (CLt n, x, (pc + 3, args), (pc + offset + 2, args))
        , state )
    | BULTINT ->
        ...
        let args = State.stack_vars state in
        ( instrs
        , Cond (CUlt n, x, (pc + offset + 2, args), (pc + 3, args))
        , state )
    | BUGEINT ->
        let n = getu32 code (pc + 1) in
        let offset = gets code (pc + 2) in
        ...
        let args = State.stack_vars state in
        ( instrs
        , Cond (CUlt n, x, (pc + 3, args), (pc + offset + 2, args))
        , state )
    | ULTINT ->
        let y = State.accu state in
        let z = State.peek 0 state in
        ...
        if debug_parser ()
        then
          Format.printf "%a = mk_bool(%a <= %a) (unsigned)@." Var.print x
            Var.print y Var.print z ;
        compile infos (pc + 1) (State.pop 1 state)
    ...
    | GETPUBMET ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = caml_get_public_method(%a, %a)@." Var.print m
            Var.print obj Var.print tag ;
        compile infos (pc + 3) state
          ( Let
              ( m
              , Prim
    ...
    | GETMETHOD ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = lookup(%a, %a)@." Var.print m Var.print obj
            Var.print lab ;
        compile infos (pc + 1) state
    ...
    AddrMap.fold
      (fun _ block map ->
        ...
        | Poptrap ((cont, _), addr_push) ->
            let set =
              try AddrSet.add cont (AddrMap.find addr_push map)
...
let parse_bytecode ~debug code globals debug_data =
  ...
  let blocks =
    Blocks.analyse
      (if debug = `Full then debug_data else Debug.create ())
      code
  in
  let blocks =
    ...
    if false && debug = `Full
    then
      Debug.fold debug_data (fun pc _ blocks -> Blocks.add blocks pc) blocks
    else blocks
  in
...
let fix_min_max_int code =
  try
    ...
    String.blit (Lazy.force fixed_code) 0 code (i + 16)
      (String.length (Lazy.force fixed_code))
  with Not_found ->
    Util.warn
      "Warning: could not fix min_int/max_int definition (bytecode not \
       found).@."
...
let really_input_string =
  (* the one above or the one in Pervasives *)
...
let exe_from_channel ~includes ?(toplevel= false) ?(expunge= fun _ -> `Keep)
    ?(dynlink= false) ~debug ~debug_data ic =
  ...
  let code =
    ...
    | `V4_02 | `V4_03 | `V4_04 | `V4_06 -> really_input_string ic code_size
  in
  ...
  let symbols =
    filter_global_map (fun id -> keep (Ident.name id)) orig_symbols
  in
  ( if debug = `No
  then ()
  else
    ...
    with Not_found ->
      ...
      | `Full ->
          Util.warn
            "Warning: Program not linked with -g, original variable names \
             and locations not availalbe.@." ) ;
  let globals =
    make_globals (Array.length init_data) init_data primitive_table
  in
  ...
  then
    ...
    Tbl.iter
      (fun id n ->
        (* Format.eprintf "export %d %d %s@." id.Ident.flags id.Ident.stamp
           id.Ident.name; *)
  ...
  let cmis =
    let exception_ids =
      List.fold_left
        (fun acc (i, _) -> max acc i)
        (-1) predefined_exceptions
    in
    if toplevel && Option.Optim.include_cmis ()
    then
      Tbl.fold
        (fun id num acc ->
          if num > exception_ids && Ident.global id
             && is_module (Ident.name id)
          then Util.StringSet.add (Ident.name id) acc
          else acc )
        symbols.num_tbl Util.StringSet.empty
    else Util.StringSet.empty
  in
...
module Reloc = struct
  ...
  let constants t =
    ...
    (* WARNING: [Obj.t array] is dangerous. Make sure we don't end up with
       an unboxed float array. *)
  ...
  let make_globals t =
    ...
    let globals =
      make_globals (Array.length constants) constants primitives
    in
    resize_globals globals t.pos ;
    Hashtbl.iter
      (fun name i -> (globals.named_value).(i) <- Some name)
      t.names ;
    (* Initialize module override mechanism *)
    List.iter
      (fun (name, v) ->
...
let from_compilation_units ~includes:_ ~debug ~debug_data l =
  ...
  | `V3 ->
      ...
      List.iter
        (fun (u, code) ->
          if u.Cmo_format.cu_name = "Pervasives" then fix_min_max_int code
          )
        l
  | `V4_02 | `V4_03 | `V4_04 | `V4_06 -> () ) ;
  let code =
      ...
      (fun i var l ->
        ...
        | Some x when (globals.is_const).(i) -> (
          ...
          | Some name ->
              ...
              Let
                ( x
                , Prim (Extern "caml_js_get", [Pv gdata; Pc (IString name)])
                )
              :: l )
        | _ -> l )
      globals.vars []
...
let from_channel ?(includes= []) ?(toplevel= false) ?expunge
  ...
  match format with
  | `Pre magic -> (
    match Util.MagicNumber.kind magic with
    | `Cmo ->
        if Option.Optim.check_magic ()
           && magic <> Util.MagicNumber.current_cmo
        ...
        let a, b, c =
          from_compilation_units ~includes ~debug ~debug_data
            [compunit, code]
        in
        a, b, c, false
    | `Cma ->
        if Option.Optim.check_magic ()
           && magic <> Util.MagicNumber.current_cma
        then raise Util.MagicNumber.(Bad_magic_version magic) ;
          ...
          List.map
            (fun compunit ->
              ...
              then ()
                ...
                seek_in ic compunit.Cmo_format.cu_debug ;
                Debug.read_event_list debug_data ~crcs:[] ~includes
                  ~orig:!orig ic ) ;
        ...
        in
        let a, b, c =
          from_compilation_units ~includes ~debug ~debug_data units
        in
        a, b, c, false
    | _ -> raise Util.MagicNumber.(Bad_magic_number (to_string magic)) )
  | `Post magic ->
    match Util.MagicNumber.kind magic with
    | `Exe ->
        if Option.Optim.check_magic ()
           && magic <> Util.MagicNumber.current_exe
        then raise Util.MagicNumber.(Bad_magic_version magic) ;
        let a, b, c =
          exe_from_channel ~includes ~toplevel ?expunge ~dynlink ~debug
            ~debug_data ic
        in
  ...
  let body =
    ...
    List.map
      (fun (index, name) ->
        ...
        ; Let
            ( v_name_js
            , Prim (Extern "caml_js_from_string", [Pc (IString name)]) )
