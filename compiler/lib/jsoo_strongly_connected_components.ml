...
module Kosaraju : sig
  ...
  let depth_first_order (graph: int list array) : int array =
    ...
    let rec aux node =
      ...
      then (
        ...
        push node )
    in
    for i = 0 to size - 1 do aux i done ;
    stack
  ...
  let component_graph graph =
    ...
    let add_component_dep node set =
      ...
      List.fold_left
        (fun set dep -> IntSet.add components.(dep) set)
        set node_deps
    in
    Array.iteri
      (fun node component ->
...
struct
  ...
  let number graph =
    ...
    let back =
      ...
      for i = 0 to size - 1 do back := Id.Map.add forth.(i) i !back done ;
      !back
    in
    let integer_graph =
      Array.init size (fun i ->
          ...
          Id.Set.fold
            (fun dest acc ->
              let v =
                try Id.Map.find dest back with Not_found -> assert false
              in
              v :: acc )
            dests [] )
    in
  ...
  let component_graph graph =
    ...
    Array.mapi
      (fun component nodes ->
        ...
        | _ :: _ ->
            ( Has_loop
                (List.map (fun node -> (numbering.forth).(node)) nodes)
            , component_edges.(component) ) )
