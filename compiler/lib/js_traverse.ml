...
class type mapper =
  object
    ...
    method statement_o :
      ...
      -> (Javascript.statement * Javascript.location) option
    method statements :
      Javascript.statement_list -> Javascript.statement_list
    method source : Javascript.source_element -> Javascript.source_element
    method sources :
      Javascript.source_elements -> Javascript.source_elements
  ...
  object (m)
    ...
    method statement s =
      ...
      | If_statement (e, (s, loc), sopt) ->
          If_statement
            (m#expression e, (m#statement s, loc), m#statement_o sopt)
      ...
      | For_statement (e1, e2, e3, (s, loc)) ->
          let e1 =
            ...
            | Right l ->
                Right
                  (List.map
                     (fun (id, eo) -> m#ident id, m#initialiser_o eo)
                     l)
          in
          For_statement
            (e1, m#expression_o e2, m#expression_o e3, (m#statement s, loc))
    ...
    method expression x =
      ...
      | EFun (idopt, params, body, nid) ->
          let idopt =
            match idopt with None -> None | Some i -> Some (m#ident i)
          in
          EFun (idopt, List.map m#ident params, m#sources body, nid)
      | EArr l -> EArr (List.map (fun x -> m#expression_o x) l)
      | EObj l -> EObj (List.map (fun (i, e) -> i, m#expression e) l)
      | (EStr _ as x)
       |(EBool _ as x)
       |(ENum _ as x)
       |(EQuote _ as x)
    ...
    method expression_o x =
      match x with None -> None | Some s -> Some (m#expression s)
    ...
    method source x =
      ...
      | Function_declaration (id, params, body, nid) ->
          Function_declaration
            (m#ident id, List.map m#ident params, m#sources body, nid)
...
class map_for_share_constant =
  object (m)
    ...
    method expression e =
      match e with
      (* JavaScript engines recognize the pattern 'typeof x==="number"'; if
         the string is shared, less efficient code is generated. *)
      ...
      (* Some js bundler get confused when the argument of 'require' is not
         a litteral *)
    ...
    method switch_case e =
      match e with ENum _ | EStr _ -> e | _ -> m#expression e
...
class replace_expr f =
  object
    ...
    method expression e =
      try EVar (f e) with Not_found -> super#expression e
  end
  ...
  object
    ...
    method program p =
      ...
      then p
        ...
        let all =
          Hashtbl.fold (fun e v acc -> (v, Some (e, N)) :: acc) all []
        in
...
class free =
  object (m: 'test)
    ...
    method merge_info from =
      ...
      let count =
        IdentMap.fold
          ...
          from#state.count m#state.count
      in
      state_ <-
        { state_ with
    ...
    method use_var x =
      ...
      | S {name; _} ->
          state_ <-
            {state_ with use_name= StringSet.add name state_.use_name; count}
    ...
    method def_var x =
      ...
      | S {name; _} ->
          state_ <-
            {state_ with def_name= StringSet.add name state_.def_name; count}
    ...
    method expression x =
      ...
      | EFun (ident, params, body, nid) ->
          ...
          let ident =
            ...
            | Some (S {name; _})
              when not (StringSet.mem name tbody#state.use_name) ->
                None
      ...
      | Try_statement (b, w, f) ->
          ...
          let w =
            ...
            | Some (id, block) ->
                ...
                in
                let def, def_name =
                  clean tbody#state.def tbody#state.def_name
                in
                let use, use_name =
                  clean tbody#state.use tbody#state.use_name
                in
                let count =
                  IdentMap.fold
                    (fun v k acc ->
                      let n =
                        try IdentMap.find v acc with Not_found -> 0
                      in
                      IdentMap.add v (k + n) acc )
                    tbody#state.count m#state.count
                in
                state_ <-
...
class rename_variable keeps =
  object
    ...
    method source x =
      ...
      | Function_declaration (id, params, body, nid) ->
          Function_declaration
            (id, List.map sub_#ident params, sub_#sources body, nid)
      | Statement _ -> x
  end
...
class compact_vardecl =
  object (m)
    ...
    method private translate_st l =
      ...
      | x :: l ->
          Expression_statement
            (List.fold_left (fun acc e -> ESeq (acc, e)) x l)
    ...
    method block ?(catch= false) params =
      ignore catch ; List.iter m#except params ; super#block params

    method merge_info from =
      super#merge_info from ;
      let all =
        S.fold
          (fun e acc -> IdentSet.add (V e) acc)
          from#state.def IdentSet.empty
      in
      let all =
        StringSet.fold
          (fun e acc -> IdentSet.add (S {name= e; var= None}) acc)
          from#state.def_name all
      in
    ...
    method private pack all sources =
      let may_flush rem vars s instr =
        ...
        then rem, [], s :: instr
          ...
          , []
          , s
            :: (Statement (Variable_statement (List.rev vars)), N)
            :: instr )
      in
      let rem, vars, instr =
        List.fold_left
          (fun (rem, vars, instr) (s, loc) ->
            match s with
            | Statement (Expression_statement e) ->
                let l = m#split e in
                List.fold_left
                  (fun (rem, vars, instr) e ->
                    match e with
                    ...
                    | x ->
                        may_flush rem vars
                          (Statement (Expression_statement x), N)
                          instr )
                  (rem, vars, instr) l
      ...
      | l, (Statement (Variable_statement l'), loc) :: rest ->
          (Statement (Variable_statement (List.rev_append l l')), loc)
          :: rest
        ...
        List.fold_left
          (fun acc (x, loc) ->
            ...
            | Expression_statement e ->
                ...
                let l =
                  List.fold_left
                    (fun acc e -> (Expression_statement e, N) :: acc)
                    acc l
                in
                l
            | _ -> (x, loc) :: acc )
  ...
  object (m)
    ...
    method statements l =
      ...
      let vars_rev, vars_loc, instr_rev =
        List.fold_left
          (fun (vars_rev, vars_loc, instr_rev) (x, loc) ->
            match x with
            | Variable_statement l when Option.Optim.compact () ->
                let vars_loc =
                  match vars_loc with Pi _ as x -> x | _ -> loc
                in
            ...
            | _ ->
                ...
                , vars_loc
                , rev_append_st (x, loc)
                    ( (Variable_statement (List.rev vars_rev), vars_loc)
                    :: instr_rev ) ) )
          ([], N, []) l
      in
      let instr_rev =
    ...
    method statement s =
      ...
      | If_statement (if', then', else') ->
          If_statement (if', b then', bopt else')
      | Do_while_statement (do', while') ->
          Do_while_statement (b do', while')
      ...
      | Switch_statement (e, l, Some [], []) ->
          Switch_statement (e, l, None, [])
      | s -> s
...
let assign_op = function
  ...
  | ( exp
    , EBin
        ( ((Div | Mod | Lsl | Asr | Lsr | Band | Bxor | Bor) as unop)
        , exp'
        , y ) )
    when exp = exp' ->
      ...
      List.fold_right
        (fun (st, loc) rem ->
          ...
          | If_statement
            ...
            when v1 = v2 ->
              ( Expression_statement (EBin (Eq, v1, ECond (cond, e1, e2)))
              , loc )
              :: rem
                ...
                List.map
                  (function
                      ...
                      | ident, Some (exp, pc) ->
                        ...
                        | None ->
                            Variable_statement [ident, Some (exp, pc)], loc)
                  l1
              in
              x @ rem
          | _ -> (st, loc) :: rem )
