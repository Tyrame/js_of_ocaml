...
let specialize_instr info (acc, free_pc, extra) i =
  ...
  | Let (x, Apply (f, l, _)) when Option.Optim.optcall () -> (
      ...
      | Some n when n = n' ->
          Let (x, Apply (f, l, true)) :: acc, free_pc, extra
      | Some n when n < n' ->
      ...
      | Some n when n > n' ->
          ...
          let block =
            let params' =
              Array.init (n - n') (fun _ -> Code.Var.fresh ())
            in
    ...
    AddrMap.fold
      (fun pc block (blocks, free_pc) ->
        let body, free_pc, extra =
          List.fold_right
            (fun i acc -> specialize_instr info acc i)
            block.body ([], free_pc, [])
        in
        let blocks =
          List.fold_left
            (fun blocks (pc, b) -> AddrMap.add pc b blocks)
            blocks extra
        in
        AddrMap.add pc {block with Code.body} blocks, free_pc )
