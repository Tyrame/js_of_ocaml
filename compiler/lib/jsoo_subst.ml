...
let expr s e =
  ...
  | Apply (f, l, n) -> Apply (s f, List.map (fun x -> s x) l, n)
  | Block (n, a) -> Block (n, Array.map (fun x -> s x) a)
  | Field (x, n) -> Field (s x, n)
  | Closure (l, pc) -> Closure (l, subst_cont s pc)
  | Prim (p, l) ->
      Prim
        ( p
        , List.map (fun x -> match x with Pv x -> Pv (s x) | Pc _ -> x) l )
...
let block s block =
  { params= block.params
  ; handler=
      Util.opt_map (fun (x, cont) -> x, subst_cont s cont) block.handler
...
let rec build_mapping params args =
  ...
  | x :: params, y :: args -> VarMap.add x y (build_mapping params args)
  | [], [] -> VarMap.empty
  | _ -> assert false

let from_map m x = try VarMap.find x m with Not_found -> x
