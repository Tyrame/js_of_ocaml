...
and property_name = PNI of identifier | PNS of string | PNN of float
...
and expression =
  ...
  (* A string can either be composed of a sequence of bytes, or be UTF-8
     encoded. In the second case, the string may contain escape sequences. *)
...
and statement =
  ...
  | If_statement of
      expression * (statement * location) * (statement * location) option
  ...
  | For_statement of
      ...
      * (statement * location)
  | ForIn_statement of
      (expression, variable_declaration) either
      * expression
      * (statement * location)
