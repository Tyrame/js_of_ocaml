  ...
  | past, Some (first, rest) ->
      ...
      let f prev x acc =
        ...
        (* When, as the program is parsed from left to right, a token is
           encountered *)
        (* that is allowed by some production of the grammar, but the
           production *)
        (* is a restricted production and the token would be the first token
           for a *)
        (* terminal or nonterminal immediately following the annotation [no
           LineTerminator here] *)
        (* within the restricted production (and therefore such a token is
           called a restricted token), *)
        (* and the restricted token is separated from the previous token by
           at least *)
...
let lexer_aux ?(rm_comment= true) lines_info lexbuf =
  let rec loop lexbuf extra lines_info prev acc =
    ...
    | _ ->
        let extra =
          match t with
          | Js_token.TComment (ii, cmt)
            when String.length cmt > 1 && cmt.[0] = '#'
            -> (
