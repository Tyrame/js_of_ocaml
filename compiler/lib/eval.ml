...
let get_static_env s =
  try Some (Hashtbl.find static_env s) with Not_found -> None

module Int = Int32

let int_binop l f =
  match l with [Int i; Int j] -> Some (Int (f i j)) | _ -> None
...
let the_length_of info x =
  get_approx info
    ...
    None
    (fun u v ->
      match u, v with Some l, Some l' when l = l' -> Some l | _ -> None )
    x
...
let eval_instr info i =
  ...
  | Let
      ( _
      , Prim (Extern ("caml_array_unsafe_get" | "caml_array_unsafe_set"), _)
      ) ->
      (* Fresh parameters can be introduced for these primitives in
         Specialize_js, which would make the call to [the_const_of] below
         fail. *)
      i
  | Let (x, Prim (IsInt, [y])) -> (
            ...
            , Prim
                ...
                , List.map2
                    (fun arg c ->
                      ...
                      | Some _
                      (* do not be duplicated other constant as they're not
                         represented with constant in javascript. *)  | None ->
                          arg )
                    prim_args prim_args' ) ) )
  | _ -> i
...
let the_case_of info x =
  ...
  | Pv x ->
      get_approx info
        (fun x ->
          match (info.info_defs).(Var.idx x) with
          | Expr (Const i) | Expr (Constant (Int i)) ->
              CConst (Int32.to_int i)
          | Expr (Block (j, _)) ->
...
let eval_branch info = function
  ...
  | Switch (x, const, tags) as b -> (
    match
      (* [the_case_of info (Pv x)] might be meaningless when we're inside a
         dead code. The proper fix would be to remove the deadcode entirely.
         Meanwhile, add guards to prevent Invalid_argument("index out of
         bounds") see https://github.com/ocsigen/js_of_ocaml/issues/485 *)
      the_case_of info (Pv x)
    with
  ...
  then visited
    ...
    List.iter
      (function
          ...
          | Let (_, e) ->
            match e with
            | Const _ | Block (_, _) | Field (_, _) | Constant _ | Closure _ ->
                ()
    ...
    | Switch (_, a1, a2) ->
        let visited =
          Array.fold_left
            (fun visited (pc, _) -> do_not_raise pc visited blocks)
            visited a1
        in
        let visited =
          Array.fold_left
