 ...
 *)
(* Helper to generate a javascript file containing cmis needed to use
   findlib packages in the toplevel *)
(* #use "topfind" *)
(* #require "findlib" *)
...
let usage () =
  ...
  Format.eprintf
    " -prefix [dir]\t\tStore cmi files in [dir] (default /static/cmis)@." ;
...
let args =
  ...
  let js, args =
    List.partition (fun s -> Filename.check_suffix s ".js") args
  in
  let js = if !runtime then "+runtime.js" :: js else js in
  let all = Jsoo_common.cmis args in
  let all =
    List.map (fun x -> Filename.(concat !prefix (basename x)), x) all
  in
  ...
  Js_of_ocaml_compiler.Driver.f pfs_fmt
