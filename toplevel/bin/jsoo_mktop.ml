 ...
 *)
(* Helper to compile js_of_ocaml toplevel with support for camlp4 syntax
   extensions *)
(* #use "topfind" *)
(* #require "findlib" *)
...
let do_clean () =
  List.iter (fun x -> if Sys.file_exists x then Sys.remove x) !to_clean
...
module Camlp4 : CAMLP4 = struct
  (* We do the same as Camlp4Bin call
     Camlp4.Register.iter_and_take_callbacks after Every load of syntax
     module to initialize them. Camlp4 do it after dynlink; Since we
     statically link modules, we have to link dummy modules to trigger the
     callbacks *)
  let flush_str =
    Printf.sprintf
      "let _ = JsooTop.register_camlp4_syntax %S \
  ...
  let with_flush x archive = [make_flush_module x; archive]
    ...
    let ll =
      List.map
        (fun x ->
          try
            "-I"
            :: Findlib.package_directory x
            :: with_flush x (Findlib.package_property preds x "archive")
          with Not_found -> [] )
        all
    in
    List.flatten ll
  ...
  let camlp4_pre4_02 =
    try
      ...
      if v.[0] >= '4' && v.[1] = '.' && v.[2] = '0' && v.[3] >= '2'
      then false
      else true
    with _ -> true

  let build ~all_pkgs ~syntax_pkgs ~syntax_mods =
    if syntax_pkgs = [] && syntax_mods = []
    then []
    else
      let maybe_add_dynlink =
        if List.mem "dynlink" all_pkgs then [] else ["dynlink.cma"]
      in
      ...
      @ [ "-I"
        ...
        ; "Camlp4Parsers/Camlp4OCamlParser.cmo" ] @ all
      @ [ ( if camlp4_pre4_02
          then "Camlp4Top/Top.cmo"
          else "jsooTopCamlp4.cmo" ) ]
end
...
let usage () =
  ...
  Format.eprintf
    " -top-syntax [pkg]\t\tInclude syntax extension provided by [pkg] \
     findlib package@." ;
  Format.eprintf
    " -top-syntax-mod [mod]\t\tInclude syntax extension provided by the \
     module [mod]@." ;
...
let _ =
  try
    ...
    in
    execute
      ( ["jsoo_mkcmis"; "-o"; "exported-unit.cmis.js"] @ !export
      @ toplevel_unit ) ;
    let export_output = !output ^ ".export" in
    let cmd =
      base_cmd
      @ Camlp4.build ~all_pkgs:!pkgs ~syntax_pkgs:!syntaxes
          ~syntax_mods:!syntaxes_mod
      @ jsoo_top @ args @ ["-o"; !output]
    in
    execute cmd ;
