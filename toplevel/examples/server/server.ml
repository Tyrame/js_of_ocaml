...
let server () =
  let re_filesys =
    compile
      (seq [str "/filesys/"; group (seq [str !filesys; rep any]); eos])
  in
  let header typ =
  ...
  let header_plain_user_charset =
    header "text/plain; charset=x-user-defined"
  in
  let callback conn_id req body =
    ...
    try
      ...
      >>= fun () ->
      Server.respond_file ~headers:header_plain_user_charset ~fname ()
    with _ ->
      (* send static file *)
