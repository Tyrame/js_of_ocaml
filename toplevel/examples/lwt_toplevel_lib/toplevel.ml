...
let setup_toplevel () =
  ...
  in
  exec'
    (Printf.sprintf "Format.printf \"%s@.\" Sys.ocaml_version;;" header1) ;
  exec' (Printf.sprintf "Format.printf \"%s@.\";;" header2) ;
  ( if JsooTop.get_camlp4_syntaxes () <> []
  then
    let header3 =
      Printf.sprintf
        "     'JsooTop.get_camlp4_syntaxes ()' to get loaded syntax \
         extensions"
    in
...
(* we need to compute the hash form href to avoid different encoding
   behavior across browser. see Url.get_fragment *)
let parse_hash () =
      ...
      e##.onclick :=
        Dom_html.handler (fun _ ->
            ...
            let frag =
              ...
              let frags =
                List.remove_assoc "code" frags @ ["code", code_encoded]
              in
            ...
            let append_url str =
              let dom =
                let open Tyxml_js.Html in
                p
                  [ pcdata "Share this url : "
                  ; a ~a:[a_href str] [pcdata str] ]
              in
...
let run _ =
  ...
  let history_up _e =
    ...
    try
      ...
      Js._true
    with Not_found ->
      History.current txt ; History.previous textbox ; Js._false
  in
  let meta e =
    let b = Js.to_bool in
