...
(** The type of event listener functions. The first type parameter ['a] is
    the type of the target object; the second parameter ['b] is the type of
    the event object. *)
...
and eventTarget =
  object ('self)
    ...
    method ondblclick :
      ('self t, mouseEvent t) event_listener writeonly_prop
    method onmousedown :
      ('self t, mouseEvent t) event_listener writeonly_prop
    ...
    method onmouseout :
      ('self t, mouseEvent t) event_listener writeonly_prop
    method onkeypress :
      ('self t, keyboardEvent t) event_listener writeonly_prop
    method onkeydown :
      ('self t, keyboardEvent t) event_listener writeonly_prop
    method onkeyup :
      ('self t, keyboardEvent t) event_listener writeonly_prop
    ...
    method ondragstart :
      ('self t, dragEvent t) event_listener writeonly_prop
...
and element =
  object
    ...
    (* Not supported by IE9 by default. Add +classList.js to the Js_of_ocaml
       command line for compatibility *)
...
class type mediaElement =
  object
    ...
    method oncanplaythrough :
      ('self t, mediaEvent t) event_listener writeonly_prop
...
class type canvasElement =
  object
    ...
    method toDataURL_type_compression :
      js_string t -> float -> js_string t meth
...
and canvasRenderingContext2D =
  object
    ...
    method createPattern :
      imageElement t -> js_string t -> canvasPattern t meth
    ...
    method drawFocusRing :
      #element t -> float -> float -> bool t -> bool t meth
    method font : js_string t prop
    method textAlign : js_string t prop
    ...
    method getImageData :
      float -> float -> float -> float -> imageData t meth
...
external pixel_set :
  canvasPixelArray t -> int -> int -> unit
  = "caml_js_set"
...
class type document =
  object
    ...
    method execCommand :
      js_string t -> bool t -> js_string t opt -> unit meth
    method createRange : range t meth
    method readyState : js_string t readonly_prop
    method getElementsByClassName :
      js_string t -> element Dom.nodeList t meth
...
(** [getElementById_coerce id coerce] returns the element with the id [id]
    in the current document and attempt to coerce it using the provided
    [coerce] function. It returns [None] if there are no such element or if
    the [coerce] function returns [Js.none]. Typical usage is the following:
    {[ match Dom_html.getElementById_coerce "myinput"
    Dom_html.CoerceTo.input with | None -> .. | Some input -> .. ]} *)
val getElementById_coerce : string -> (element t -> 'a opt) -> 'a option

(** [getElementById id] returns the element with the id [id] in the current
    document. It raises [Not_found] if there are no such element *)
...
class type history =
  object
    ...
    method replaceState :
      'a. 'a -> js_string t -> js_string t opt -> unit meth
  end 
...
class type window =
  object
    ...
    method open_ :
      js_string t -> js_string t -> js_string t opt -> window t opt meth
    ...
    method setInterval :
      (unit -> unit) Js.callback -> float -> interval_id meth
    method clearInterval : interval_id -> unit meth
    method setTimeout :
      (unit -> unit) Js.callback -> float -> timeout_id meth
...
val full_handler :
  ('a -> (#event t as 'b) -> bool t) -> ('a, 'b) event_listener
...
(** Add an event listener. This function matches the [addEventListener] DOM
    method, except that it returns an id for removing the listener. *)
...
(** Add a mousewheel event listener. The callback is provided the event and
    the numbers of ticks the mouse wheel moved. Positive means down / right. *)
...
(** Use [Keyboard_code] when you want to identify a key that the user
    pressed. This should be invoked for keydown and keyup events, not
    keypress events.

    If the browser supports the standardized [key] and [code] properties,
    then [of_event] on a keypress event will have the correct behavior.
    Otherwise, it might not identify or might mis-identify which key was
    pressed. This occurs because the keypress event is designed for
    printable characters while keydown and keyup are designed for physical
    keys. Thus, the older properties of [keyEvent] change behavior between
    keydown and keypress events. This change in behavior is what causes the
    mapping to be incorrect. *)
...
(** Use [Keyboard_key] when you want to identify the character that the user
    typed. This should only be invoked on keypress events, not keydown or
    keyup events. *)
...
(** @raise Canvas_not_available when canvas elements are not supported by
    the browser. *)
...
(** Coercion from a general DOM element to an HTML element. (Unsafe in
    general.) *)
...
(** Same as [Dom_html.window##setTimeout cb ms] but prevents overflow with
    delay greater than 24 days. *)
...
val js_array_of_collection :
  #element collection Js.t -> #element Js.t Js.js_array Js.t

(** {2 Deprecated function.} *)

(** Call the appropriate [requestAnimationFrame] method variant (depending
    on the navigator), or sleep for a short amount of time when there no
    such method is provided. We currently prefix the function name with as
    underscore as the interface of this function is not completely
    standardized yet. Thus, we leave the room to a function with a possibly
    refined type.

    This function is deprecated. Use the requestAnimationFrame of the window
    object instead. *)
