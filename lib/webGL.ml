...
class type renderingContext =
  object
    ...
    method getBufferParameter :
      'a. bufferTarget -> 'a bufferParameter -> 'a meth
    ...
    method bindAttribLocation :
      program t -> uint -> js_string t -> unit meth
    ...
    method getUniformLocation :
      'a. program t -> js_string t -> 'a uniformLocation t meth
    method getVertexAttrib : 'a. uint -> 'a vertexAttribParam -> 'a meth
    method getVertexAttribOffset :
      uint -> vertexAttribPointerParam -> sizeiptr meth
    ...
    method uniform1fv :
      float uniformLocation t -> float js_array t -> unit meth
    ...
    method uniform2f :
      [`vec2] uniformLocation t -> float -> float -> unit meth
    method uniform2fv_typed :
      [`vec2] uniformLocation t -> Typed_array.float32Array t -> unit meth
    method uniform2fv :
      [`vec2] uniformLocation t -> float js_array t -> unit meth
    method uniform2i : [`ivec2] uniformLocation t -> int -> int -> unit meth
    method uniform2iv :
      [`ivec2] uniformLocation t -> int js_array t -> unit meth
    method uniform2iv_typed :
      [`ivec2] uniformLocation t -> Typed_array.int32Array t -> unit meth
    method uniform3f :
      [`vec3] uniformLocation t -> float -> float -> float -> unit meth
    method uniform3fv_typed :
      [`vec3] uniformLocation t -> Typed_array.float32Array t -> unit meth
    method uniform3fv :
      [`vec3] uniformLocation t -> float js_array t -> unit meth
    method uniform3i :
      [`ivec3] uniformLocation t -> int -> int -> int -> unit meth
    method uniform3iv :
      [`ivec3] uniformLocation t -> int js_array t -> unit meth
    ...
    method uniform4fv :
      [`vec4] uniformLocation t -> float js_array t -> unit meth
    method uniform4i :
      [`ivec4] uniformLocation t -> int -> int -> int -> int -> unit meth
    method uniform4iv :
      [`ivec4] uniformLocation t -> int js_array t -> unit meth
    ...
    method vertexAttrib1fv_typed :
      uint -> Typed_array.float32Array t -> unit meth
    ...
    method vertexAttrib4f :
      uint -> float -> float -> float -> float -> unit meth
    ...
    method drawElements :
      beginMode -> sizei -> dataType -> intptr -> unit meth
    ...
    method _ELEMENT_ARRAY_BUFFER_BINDING_ :
      buffer t opt parameter readonly_prop
    ...
    method _MAX_VIEWPORT_DIMS_ :
      Typed_array.int32Array t parameter readonly_prop
    ...
    method _TEXTURE_BINDING_CUBE_MAP_ :
      texture t opt parameter readonly_prop
    ...
    method _RENDERBUFFER_BINDING_ :
      renderbuffer t opt parameter readonly_prop
    ...
    method _VERTEX_ATTRIB_ARRAY_POINTER_ :
      vertexAttribPointerParam readonly_prop
    ...
    method _UNPACK_PREMULTIPLY_ALPHA_WEBGL_PARAM :
      bool t parameter readonly_prop
    ...
    method _UNPACK_PREMULTIPLY_ALPHA_WEBGL_ :
      bool t pixelStoreParam readonly_prop
    method _UNPACK_COLORSPACE_CONVERSION_WEBGL_ :
      int pixelStoreParam readonly_prop
  end 
...
module Event = struct
  ...
  let webglcontextcreationerror =
    Dom_html.Event.make "webglcontextcreationerror"
end
...
let getContext (c: Dom_html.canvasElement t) =
  ...
  if Opt.test ctx
  then ctx
  else c ## (getContext (Js.string "experimental-webgl"))
...
let getContextWithAttributes (c: Dom_html.canvasElement t) attribs =
  ...
  if Opt.test ctx
  then ctx
  else c##getContext_ (Js.string "experimental-webgl") attribs
