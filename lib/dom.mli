...
and element =
  object
    ...
    method getAttributeNS :
      js_string t -> js_string t -> js_string t opt meth
    method setAttributeNS :
      js_string t -> js_string t -> js_string t -> unit meth
    ...
    method getAttributeNodeNS :
      js_string t -> js_string t -> attr t opt meth
...
(** [insertBefore p n c] inserts node [n] as child of node [p], just before
    node [c], or as last child if [p] is empty. The expression [insertBefore
    n c p] behave the same as [p##insertBefore n c] but avoid the need of
    coercing the different objects to [node t]. *)
val insertBefore : #node t -> #node t -> #node t opt -> unit

(** The expression [replaceChild p n c] behave the same as [p##replaceChild
    n c] (replace [c] by [n] in [p]) but avoid the need of coercing the
    different objects to [node t]. *)
val replaceChild : #node t -> #node t -> #node t -> unit

(** The expression [removeChild n c] behave the same as [n##removeChild c]
    (remove [c] from [n]) but avoid the need of coercing the different
    objects to [node t]. *)
val removeChild : #node t -> #node t -> unit

(** The expression [appendChild n c] behave the same as [n##appendChild c]
    (appends [c] to [n]) but avoid the need of coercing the different
    objects to [node t]. *)
...
(** Create an event handler that invokes the provided function. If the
    handler returns false, the default action is prevented. *)
val handler : (('e #event t as 'b) -> bool t) -> ('a, 'b) event_listener

(** Create an event handler that invokes the provided function. The event
    target (implicit parameter [this]) is also passed as argument to the
    function. *)
val full_handler :
  ('a -> ('e #event t as 'b) -> bool t) -> ('a, 'b) event_listener
...
(** Call this to prevent the default handler for the event. To stop
    propagation of the event, call {!Dom_html.stopPropagation}. *)
