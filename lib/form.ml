              ...
              then [name, `String (Js.string "")]
                ...
                | Some true ->
                    filter_map
                      ...
                      (Array.to_list
                         (Array.init list##.length (fun i -> list##item i)))
        )
    | _ -> [name, `String value]
  else []
...
let form_elements ?get (form: formElement t) =
  ...
  let elements =
    Array.to_list
      (Array.init length (fun i -> Opt.to_option (form##.elements##item i)))
  in
  let contents =
    List.flatten
      (List.map
...
let get_form_contents form =
  List.map
    (function name, `String s -> name, to_string s | _ -> assert false)
