...
let empty_mutation_observer_init () : mutationObserverInit Js.t =
  Js.Unsafe.obj [||]
...
let observe ~(node: #Dom.node Js.t)
  ...
  let () =
    opt_iter attribute_old_value (fun v -> cfg##.attributeOldValue := v)
  in
  let () =
    opt_iter character_data_old_value (fun v ->
        cfg##.characterDataOldValue := v )
  in
  let () =
