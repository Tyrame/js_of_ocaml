...
external decode_html_entities :
  js_string t -> js_string t
...
let getElementById_exn id =
  Js.Opt.case
    (document##getElementById (Js.string id))
    (fun () ->
      failwith (Printf.sprintf "getElementById_exn: %S not found" id) )
    (fun pnode -> pnode)
  ...
  then Js.Unsafe.coerce (createElement doc elt)
    ...
    | `Extended ->
        ...
        opt_iter _type (fun t ->
            ignore
              (a##push_3 (Js.string " type=\"") (html_escape t)
                 (Js.string "\"")) ) ;
        opt_iter name (fun n ->
            ignore
              (a##push_3 (Js.string " name=\"") (html_escape n)
                 (Js.string "\"")) ) ;
...
let createOptgroup doc : optGroupElement t =
...
let createFieldset doc : fieldSetElement t =
...
let createColgroup doc : tableColElement t =
  unsafeCreateElement doc "colgroup"

let createThead doc : tableSectionElement t =
  unsafeCreateElement doc "thead"

let createTfoot doc : tableSectionElement t =
  unsafeCreateElement doc "tfoot"

let createTbody doc : tableSectionElement t =
...
let createFrameset doc : frameSetElement t =
  unsafeCreateElement doc "frameset"
...
module CoerceTo = struct
  let element : #Dom.node Js.t -> element Js.t Js.opt =
    ...
    then
      (* ie < 9 does not have HTMLElement: we have to cheat to check that
         something is an html element *)
      fun e ->
      ...
      fun e ->
      if Js.instanceof e html_element
  ...
  then
    addEventListener e Event.mousewheel
      (handler (fun (e: mousewheelEvent t) ->
           ...
           let dy =
             -Optdef.get e##.wheelDeltaY (fun () -> e##.wheelDelta) / 40
           in
...
module Keyboard_key = struct
  ...
  let char_of_int value =
    if 0 < value
    then try Some (Uchar.of_int value) with _ -> None
    else None
...
let tagged (e: #element t) =
  ...
  then other e
    ...
    | 'd' -> (
      ...
      | _ -> other e )
    | 'e' -> (
      match tag with "embed" -> Embed (Js.Unsafe.coerce e) | _ -> other e )
    | 'f' -> (
      match tag with
    ...
    | 'p' -> (
      ...
      | _ -> other e )
    | 'q' -> (
      match tag with "q" -> Q (Js.Unsafe.coerce e) | _ -> other e )
    | 's' -> (
      match tag with
    ...
    | 't' -> (
      ...
      | _ -> other e )
    | 'u' -> (
      match tag with "ul" -> Ul (Js.Unsafe.coerce e) | _ -> other e )
    | 'v' -> (
      match tag with "video" -> Video (Js.Unsafe.coerce e) | _ -> other e )
    | _ -> other e
  ...
  Js.Opt.case (CoerceTo.mouseEvent ev)
    (fun () ->
      Js.Opt.case
        ...
        (fun () ->
          Js.Opt.case (CoerceTo.wheelEvent ev)
            (fun () ->
              Js.Opt.case
...
let opt_taggedEvent ev =
  Opt.case ev (fun () -> None) (fun ev -> Some (taggedEvent ev))

let stopPropagation ev =
  let e = Js.Unsafe.coerce ev in
  Optdef.case e##.stopPropagation
