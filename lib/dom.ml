...
let handler f =
  Js.some
    (Js.Unsafe.callback (fun e ->
         (* depending on the internet explorer version, e can be null or
            undefined. *)
...
let invoke_handler (f: ('a, 'b) event_listener) (this: 'a) (event: 'b) :
    bool t =
...
let eventTarget (e: (< .. > as 'a) #event t) : 'a t =
  ...
  then
    ...
    then
      Js.Unsafe.coerce
        (Opt.get target'##.parentNode (fun () -> assert false))
    else target
  else target
