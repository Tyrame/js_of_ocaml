...
let blunt_str_array_get a i =
  Js.to_bytestring
    (Js.Optdef.get (Js.array_get a i) (fun () -> assert false))
...
let list_of_js_array a =
  let rec aux accu idx =
    if idx < 0
    then accu
    else aux (blunt_str_array_get a idx :: accu) (idx - 1)
  in
  aux [] (a##.length - 1)
...
let bounded_split r s i =
  r##.lastIndex := 0 ;
  list_of_js_array
    (Js.str_array (Js.bytestring s) ## (split_regExpLimited r i))
...
let quote s =
  Js.to_bytestring
    (Js.bytestring s) ## (replace quote_re (Js.string "\\$&"))
