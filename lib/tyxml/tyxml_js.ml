...
module Xml = struct
  ...
  let float_attrib name value : attrib =
    attr name (js_string_of_float value)
  ...
  let space_sep_attrib name values =
    attr name (Js.string (String.concat " " values))
  ...
  let comment c =
    (Dom_html.document##createComment (Js.string c) :> Dom.node Js.t)
  ...
  let entity e =
    let entity =
      Dom_html.decode_html_entities (Js.string ("&" ^ e ^ ";"))
    in
    (Dom_html.document##createTextNode entity :> Dom.node Js.t)

  (* TODO: fix get_prop it only work when html attribute and dom property
     names correspond. find a way to get dom property name corresponding to
     html attribute *)
  ...
  let attach_attribs node l =
    List.iter
      (fun (n', att) ->
        ...
        match att with
        | Attr a ->
            (* Note that once we have weak pointers working, we'll need to
               React.S.retain *)
...
module Xml_Svg = struct
  ...
  let leaf ?(a= []) name =
    let e =
      Dom_html.document##createElementNS Dom_svg.xmlns (Js.string name)
    in
    attach_attribs e a ;
    (e :> Dom.node Js.t)
...
module Util = struct
  ...
  let merge_one_patch (dom: Dom.node Js.t) (p: Dom.node Js.t p) =
    ...
    | X (i, move) ->
        ...
        then ()
          ...
          match Js.Opt.to_option (dom##.childNodes##item i) with
          | Some i' ->
              insertAt dom (i + if move > 0 then move + 1 else move) i'
          | _ -> assert false
  ...
  let update_children (dom: Dom.node Js.t) (nodes: Dom.node Js.t t) =
    ...
    (* Note that once we have weak pointers working, we'll need to
       React.S.retain *)
    let _s : unit React.S.t =
      fold (fun () msg -> merge_msg dom msg) nodes ()
    in
    ()
end
...
module R = struct
  let filter_attrib (name, a) on =
    match a with
    | Xml.Event _ | Xml.MouseEvent _ | Xml.KeyboardEvent _ ->
        raise
          (Invalid_argument
             "filter_attrib not implemented for event handler")
    | Xml.Attr a ->
        ( name
        , Xml.Attr (React.S.l2 (fun on a -> if on then a else None) on a) )
  ...
  module Xml = struct
    ...
    let mouse_event_handler_attrib name s =
      Xml.mouse_event_handler_attrib name s

    let keyboard_event_handler_attrib name s =
      Xml.keyboard_event_handler_attrib name s
