...
(*let to_channel t oc v = let buf = Buffer.create 50 in t.write buf v;
  Buffer.output_buffer oc buf *)

let from_string t s = t.read (Lexer.init_lexer (Lexing.from_string s))

(* let from_channel t ic = t.read (Lexer.init_lexer (Lexing.from_channel
   ic)) *)
...
module Json_int64 = Defaults (struct
  ...
  let write buffer i =
    Printf.bprintf buffer "[255,%Ld,%Ld,%Ld]" (Int64.logand i mask24)
...
module Json_string = Defaults (struct
  (* Given that JSON must be valid UTF-8 and that OCaml string are just a
     sequence of byte we need to "embed" byte string in an UTF-8 sequence.
     Each byte af an OCaml string is considered as Unicode code point (<
     256) and then encoded in UTF-8. Hence, bytes greater than 127 are
     "wrapped" in two bytes. *)
  type a = string
  ...
  let write buffer s =
    ...
    for i = 0 to String.length s - 1 do
      ...
      | _c (* >= '\x80' *) ->
          ...
          Buffer.add_char buffer
            (Char.chr (0xC2 lor (Char.code s.[i] lsr 6))) ;
          Buffer.add_char buffer
            (Char.chr (0x80 lor (Char.code s.[i] land 0x3F)))
    done ;
    Buffer.add_char buffer '\"'
...
let read_list f buf =
  let rec aux l c =
    ...
    | `Cst 0 ->
        for _i = c downto 1 do Lexer.read_rbracket buf done ;
        List.rev l
    | `NCst 0 ->
        Lexer.read_comma buf ;
        let x = f buf in
...
let write_list f buffer xs =
  let rec aux l c =
    ...
    | [] ->
        ...
        for _i = c downto 1 do Buffer.add_char buffer ']' done
    | x :: xs ->
