...
(** This module provides functions for tampering with Url. It's main goal is
    to allow one to stay in the Ocaml realm without wandering into the
    {!Dom_html.window}##.location object. *)

(** The first functions are mainly from and to string conversion functions
    for the different parts of a url. *)
...
(** [urlencode ?with_plus s] replace characters for their percent encoding
    representation. Note that the '/' (slash) character is escaped as well.
    If [with_plus] is [true] (default) then ['+']'s are escaped as ["%2B"].
    If not, ['+']'s are left as is. *)
...
(** [decode_arguments s] parses [s] returning the sliced-diced association
    list. *)
...
(** [url_of_string s] parses [s] and builds a value of type [url] if [s] is
    not a valid url string, it returns [None]. *)
val url_of_string : string -> url option

(** [string_of_url u] returns a valid string representation of [u]. Note
    that * [string_of_url ((fun Some u -> u) (url_of_string s))] is NOT
    necessarily * equal to [s]. However [url_of_string (string_of_url u) =
    u]. *)
...
module Current : sig
  ...
  (** [as_string] is the original string representation of the current Url.
      It is NOT necessarily equals to [string_of_url (get ())] but
