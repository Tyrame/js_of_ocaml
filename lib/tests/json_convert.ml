...
(* let log_stop = log_start "Json"
   ...
   let str = let b = Buffer.create 256 in for i = 0 to 255 do
   Buffer.add_char b (Char.chr i) done; Buffer.contents b
   ...
   equal"; if v = Json.unsafe_input (Js.string (Deriving_Json.to_string t
   v)) then log_success () else log_failure "Not equal"; if v =
   Deriving_Json.from_string t (Deriving_Json.to_string t v) then
   log_success () else log_failure "Not equal"
   ...
   include Deriving_Json.Convert(struct type a = A.a t type b = A.a
   StringMap.t let t = S.t let to_ : b -> A.a t = fun a -> Array.of_list
   ...
   (* module T2 = T.Json_string_map_t(Json_t);; *) (* type u = int
   StringMap.t deriving (Json) *)
