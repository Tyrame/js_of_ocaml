...
and animatedNumberList =
  [numberList t] animated 
...
and animatedLengthList =
  [lengthList t] animated 
...
and defsElement =
  object
    ...
    inherit externalResourcesRequired
    inherit stylable
    inherit transformable (* XXXXXXX ? inherit Dom_html.eventTarget *)
  end 
...
and descElement =
  object
    inherit element
    inherit langSpace
    inherit stylable (* XXXXXXX ? inherit Dom_html.eventTarget *)
  end 
...
(* XXX deprecated => interface GetSVGDocument => SVGDocument
   getSVGDocument() *)
...
and animatedTransformList =
  [transformList t] animated 
...
and animatedPreserveAspectRatio =
  [preserveAspectRatio t] animated 
...
and textPathElementMethod =
  [textPathMethodType] animated 

and textPathElementSpacing =
  [textPathSpacingType] animated 
...
and altGlyphDefElement =
  element 

(* interface SVGAltGlyphItemElement *)
and altGlyphItemElement =
  element 
...
(* void setPaint(in unsigned short paintType, in DOMString uri, in DOMString
   rgbColor, in DOMString iccColor) raises(SVGException); *)
(* }; *)
...
and animatedSpreadMethod =
  [spreadMethodType] animated 
...
class type fontFaceElement =
  element 

(* interface SVGFontFaceSrcElement *)
class type fontFaceSrcElement =
  element 

(* interface SVGFontFaceUriElement *)
class type fontFaceUriElement =
  element 

(* interface SVGFontFaceFormatElement *)
class type fontFaceFormatElement =
  element 

(* interface SVGFontFaceNameElement *)
class type fontFaceNameElement =
  element 

(* interface SVGMetadataElement *)
class type metadataElement =
  element 
...
let createAltGlyph doc : altGlyphElement t =
  unsafeCreateElement doc "altglyph"

let createAltGlyphDef doc : altGlyphDefElement t =
  unsafeCreateElement doc "altglyphdef"
...
let createClipPath doc : clipPathElement t =
  unsafeCreateElement doc "clippath"
...
let createFontFaceFormat doc : fontElement t =
  unsafeCreateElement doc "font-face-format"

let createFontFaceName doc : fontElement t =
  unsafeCreateElement doc "font-face-name"

let createFontFaceSrc doc : fontElement t =
  unsafeCreateElement doc "font-face-src"

let createFontFaceUri doc : fontElement t =
  unsafeCreateElement doc "font-face-uri"
...
let createMetaData doc : metadataElement t =
  unsafeCreateElement doc "metadata"
...
let createParttern doc : patternElement t =
  unsafeCreateElement doc "parttern"

let createPolygon doc : polygonElement t = unsafeCreateElement doc "polygon"

let createPolyline doc : polyLineElement t =
  unsafeCreateElement doc "polyline"
...
let createTextpath doc : textPathElement t =
  unsafeCreateElement doc "textpath"
...
module CoerceTo = struct
  let element (e: #Dom.node Js.t) : element Js.t Js.opt =
    if Js.instanceof e svg_element
  ...
  let altGlyphDef e : altGlyphDefElement t opt =
    unsafeCoerce e "altglyphdef"

  let altGlyphItem e : altGlyphItemElement t opt =
    unsafeCoerce e "altglyphitem"

  let animate e : animateElement t opt = unsafeCoerce e "animate"

  let animateColor e : animateColorElement t opt =
    unsafeCoerce e "animatecolor"

  let animateMotion e : animateMotionElement t opt =
    unsafeCoerce e "animatemotion"
  ...
  (* let ColorProfile e : colorProfile t opt = unsafeCoerce e
     "color-profile" *)
  ...
  let fontFaceFormat e : fontElement t opt =
    unsafeCoerce e "font-face-format"
