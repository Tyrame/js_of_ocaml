...
(** Set a callback to be called when an in_channel wants to fill its buffer.
    [set_channel_filler chan cb] install the called [cb] for [chan]
    in_channel. The string returned by [cb] will be appended to the channel
    buffer. *)
...
(** Register a callback to the [path] to dynamicly load missing files.
    ...
    get the content of the file. [mount ~path f] register the callback [f]
    to the path [path]. The callback [f] receives [(prefix,suffix)] where: -
...
(** [read_file name] returns the content of the file [name]. Raise
    [Not_found] if the file does not exists. *)
val read_file : name:string -> string

(** Register a file to a Pseudo Filesystem. [create_file ~name ~content]
    register the file [name] with content [content] so it can be be opened
    with [Pervasives.open_in name] *)
val create_file : name:string -> content:string -> unit

(** Update a file in the Pseudo Filesystem. [update_file ~name ~content]
    update the file [name] with content [content] *)
val update_file : name:string -> content:string -> unit

(** {2 Information.} *)

(** [js_of_ocaml_version] is the version of Js_of_ocaml. It is a string of
    the form ["major.minor[.patchlevel][+additional-info]"], where [major],
    [minor], and [patchlevel] are integers, and [additional-info] is an
    arbitrary string. The [[.patchlevel]] and [[+additional-info]] parts may
    be absent. *)
