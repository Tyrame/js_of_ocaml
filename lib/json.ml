...
class type json =
  object
    ...
    (* Beware that this is only works when the function argument expects
       exactly two arguments (no curryfication is performed). *)
    method stringify_ :
      'a 'b 'c 'd. 'a -> (js_string t -> 'c -> 'd) -> js_string t meth
  end 
