...
let array_mapi f a =
  array_map_poly a (wrap_callback (fun x idx _ -> f idx x))
...
let date_min : (int -> int -> int -> int -> int -> date t) constr =
  date_constr
...
let raise_js_error : error t -> 'a =
  Unsafe.js_expr "(function (exn) { throw exn })"
...
let _ =
  Printexc.register_printer (fun e ->
      ...
      if instanceof e array_constructor
      then None
...
let export_all obj =
  ...
  keys##forEach
    (wrap_callback (fun (key: js_string t) _ _ ->
