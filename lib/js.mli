...
(** Signatures of a set of standard functions for manipulating optional
    values. *)
module type OPT = sig
  type 'a t
  ...
  (** Get the value. If no value available, an alternative function is
      called to get a default value. *)
...
(** Type used to specify method types: a Javascript object [<m : t1 -> t2 ->
    ... -> tn -> t Js.meth> Js.t] has a Javascript method [m] expecting {i
    n} arguments of types [t1] to [tn] and returns a value of type [t]. *)
type +'a meth

(** Type used to specify the properties of Javascript objects. In practice
    you should rarely need this type directly, but should rather use the
    type abbreviations below instead. *)
type +'a gen_prop

(** Type of read-only properties: a Javascript object [<p : t
    Js.readonly_prop> Js.t] has a read-only property [p] of type [t]. *)
...
(** Type of read/write properties that may be undefined: you can set them to
    a value of some type [t], but if you read them, you will get a value of
    type [t optdef] (that may be [undefined]). *)
type 'a optdef_prop = <get: 'a optdef ; set: 'a -> unit> gen_prop

(** {2 Object constructors} *)

(** A value of type [(t1 -> ... -> tn -> t Js.t) Js.constr] is a Javascript
    constructor expecting {i n} arguments of types [t1] to [tn] and
    returning a Javascript object of type [t Js.t]. Use the syntax extension
    [jsnew c (e1, ..., en)] to build an object using constructor [c] and
    arguments [e1] to [en]. *)
type +'a constr

(** {2 Callbacks to OCaml} *)

(** Type of callback functions. A function of type [(u, t1 -> ... -> tn ->
    t) meth_callback] can be called from Javascript with [this] bound to a
    value of type [u] and up to {i n} arguments of types [t1] to [tn]. The
    system takes care of currification, so less than {i n} arguments can be
    provided. As a special case, a callback of type [(t, unit -> t)
    meth_callback] can be called from Javascript with no argument. It will
    behave as if it was called with a single argument of type [unit]. *)
...
(** Opaque type for string arrays. You can get the actual [Array] object
    using function [Js.str_array]. (This type is used to resolved the mutual
...
(** Constructor of [RegExp] objects. The expression [jsnew regExp (r)]
...
class type ['a] js_array =
  object
    ...
    method splice_4 :
      int -> int -> 'a -> 'a -> 'a -> 'a -> 'a js_array t meth
    ...
    method every :
      ('a -> int -> 'a js_array t -> bool t) callback -> bool t meth
    method forEach :
      ('a -> int -> 'a js_array t -> unit) callback -> unit meth
    method map :
...
(** Array map: [array_map f a] is [a##map(wrap_callback (fun elt idx arr ->
    f elt))]. *)
val array_map : ('a -> 'b) -> 'a #js_array t -> 'b #js_array t

(** Array mapi: [array_mapi f a] is [a##map(wrap_callback (fun elt idx arr
    -> f idx elt))]. *)
...
(** Convert an opaque [string_array t] object into an array of string. (Used
    to resolved the mutual dependency between string and array type
    definitions.) *)
val str_array : string_array t -> js_string t js_array t

(** Convert a match result handle into a [MatchResult] object. (Used to
    resolved the mutual dependency between string and array type
    definitions.) *)
...
(** Constructor of [Date] objects: [jsnew date_now ()] returns a [Date]
...
(** Constructor of [Date] objects: [jsnew date_fromTimeValue (y, m)] returns
    a [Date] object corresponding to year [y] and month [m]. *)
val date_month : (int -> int -> date t) constr

(** Constructor of [Date] objects: [jsnew date_fromTimeValue (y, m, d)]
    returns a [Date] object corresponding to year [y], month [m] and day
    [d]. *)
...
(** Constructor of [Date] objects: [jsnew date_fromTimeValue (y, m, d, h,
    m')] returns a [Date] object corresponding to year [y] to minute [m']. *)
val date_min : (int -> int -> int -> int -> int -> date t) constr

(** Constructor of [Date] objects: [jsnew date_fromTimeValue (y, m, d, h,
    m', s)] returns a [Date] object corresponding to year [y] to second [s]. *)
val date_sec : (int -> int -> int -> int -> int -> int -> date t) constr

(** Constructor of [Date] objects: [jsnew date_fromTimeValue (y, m, d, h,
    m', s, ms)] returns a [Date] object corresponding to year [y] to
    millisecond [ms]. *)
val date_ms :
...
(** Decode a URI: replace by the corresponding byte all escape sequences but
    the ones corresponding to a URI reserved character and convert the
...
(** Encode a URI: convert the string to UTF-8 and replace all unsafe bytes
...
(** Unescape a string: 2-digit and 4-digit escape sequences are replaced by
    the corresponding UTF-16 code point. *)
...
(** Conversion of strings of bytes from OCaml to Javascript. (Each byte will
    be converted in an UTF-16 code point.) *)
...
(** Apply a possibly failing coercion function. [coerce v c f] attempts to
    apply coercion [c] to value [v]. If the coercion returns [null],
    function [f] is called. *)
val coerce : 'a -> ('a -> 'b Opt.t) -> ('a -> 'b) -> 'b

(** Apply a possibly failing coercion function. [coerce_opt v c f] attempts
    to apply coercion [c] to value [v]. If [v] is [null] or the coercion
    returns [null], function [f] is called. Typical usage is the following:
...
(** Invokes any available debugging functionality. If no debugging
    functionality is available, it has no effect. In practice, it will
    insert a "debugger;" statement in the generated javascript. *)
external debugger : unit -> unit = "debugger"

(** {2 Export functionality.} Export values to [module.exports] if it exists
    or to the global object otherwise. *)

(** [export name value] export [name] *)
val export : string -> 'a -> unit

(** [export_all obj] export every key of [obj] object. {[ export_all
    object%js method add x y = x +. y method abs x = abs_float x val zero =
    0. end ]} *)
...
module Unsafe : sig
  ...
  (** Get the value of an object property. The expression [get o s] returns
      the value of property [s] of object [o]. *)
  external get : 'a -> 'b -> 'c = "caml_js_get"

  (** Set an object property. The expression [set o s v] set the property
      [s] of object [o] to value [v]. *)
  external set : 'a -> 'b -> 'c -> unit = "caml_js_set"

  (** Delete an object property. The expression [delete o s] deletes
  ...
  (** Asserts that an expression is pure, and can therefore be optimized
      away by the compiler if unused. *)
  external pure_expr : (unit -> 'a) -> 'a = "caml_js_pure_expr"

  (** Evaluate Javascript code *)
  external eval_string : string -> 'a = "caml_js_eval_string"

  (** [js_expr e] will parse the JavaScript expression [e] if [e] is
      available at compile time or will fallback to a runtime evaluation.
      See [eval_string] *)
  ...
  (** Wrap an OCaml function so that it can be invoked from Javascript.
      Contrary to [Js.wrap_callback], partial application and
      over-application are not supported: missing arguments will be set to
      [undefined] and extra arguments are lost. *)
  external callback :
    ('a -> 'b) -> ('c, 'a -> 'b) meth_callback
    = "%identity"
  ...
  (** Wrap an OCaml function so that it can be invoked from Javascript. The
      first parameter of the function will be bound to the value of the
      [this] implicit parameter. Contrary to [Js.wrap_meth_callback],
      partial application and over-application is not supported: missing
      arguments will be set to [undefined] and extra arguments are lost. *)
  ...
  (** Wrap an OCaml function so that it can be invoked from Javascript. The
      first parameter of the function will be bound to the value of the
      [this] implicit parameter. The second parameter of the function with
      be bound to the value of the [arguments]. *)
