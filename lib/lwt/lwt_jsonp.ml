...
let raw_call name uri error_cb user_cb =
  ...
  let finalize () =
    ...
    Js.Opt.iter script##.parentNode (fun parent ->
        Dom.removeChild parent script )
  in
...
let call_custom_url ?timeout ?(prefix= "") make_uri =
  let t, w = Lwt.task () in
  let init, finalize =
    call_ prefix make_uri (fun _ -> Lwt.cancel t) (Lwt.wakeup w)
  in
  ...
  let new_t =
    ...
    | Some delay ->
        let wait =
          Lwt.bind (Lwt_js.sleep delay) (fun () -> Lwt.cancel t ; t)
        in
        Lwt.choose [wait; t]
  in
  init () ; new_t
        ...
        let new_url =
          ...
          | Url.File file ->
              Url.File
                { file with
                  Url.fu_arguments=
                    add_param param cbname file.Url.fu_arguments }
        in
