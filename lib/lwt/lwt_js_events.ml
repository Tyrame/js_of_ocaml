...
let make_event event_kind ?(use_capture= false) target =
  ...
  el :=
    Js.some
      (Dom.addEventListener target event_kind
         (Dom_html.handler (fun (ev: #Dom_html.event Js.t) ->
              cancel () ; Lwt.wakeup w ev ; Js.bool true ))
         (* true because we do not want to prevent default -> the user can
            use the preventDefault function above. *)
         (Js.bool use_capture)) ;
  t
...
let catch_cancel f x =
  Lwt.catch
    (fun () -> f x)
    (function Lwt.Canceled -> Lwt.return () | e -> Lwt.fail e)
...
let seq_loop evh ?(cancel_handler= false) ?use_capture target handler =
  ...
  let rec aux () =
    if not !cancelled
       (* In the case it has been cancelled during the previous handler, we
          do not reinstall the event handler *)
    then (
...
let func_limited_loop event limited_func ?use_capture target handler =
  ...
  async_loop event ?use_capture target (fun ev lt ->
      ...
      let nb = !count in
      limited_func ()
      >>= fun _ -> if !count = nb then handler ev lt else Lwt.return () )
...
let mousewheel ?(use_capture= false) target =
  ...
  el :=
    Js.some
      (Dom_html.addMousewheelEventListener target
...
(* let _DOMMouseScroll ?use_capture target = make_event
   Dom_html.Event._DOMMouseScroll ?use_capture target *)
...
let transitionend elt =
  ...
  | [] -> Lwt.return ()
  | l ->
      Lwt.pick (List.map (fun ev -> make_event ev elt) l)
      >>= fun _ -> Lwt.return ()
...
let onbeforeunload () =
  make_event Dom_html.Event.beforeunload Dom_html.window
...
let onhashchanges t =
  seq_loop (fun ?use_capture:_ () -> onhashchange ()) () t
...
let limited_onorientationchanges ?elapsed_time t =
  limited_loop
    (fun ?use_capture:_ () -> onorientationchange ())
    ?elapsed_time () t
