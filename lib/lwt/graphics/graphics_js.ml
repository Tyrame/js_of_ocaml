...
let _ =
  Callback.register_exception "Graphics.Graphic_failure"
    (Graphic_failure "")
...
let read_key () =
  ...
  Lwt_js_events.keypress doc
  >>= fun ev -> Lwt.return (Char.chr ev##.keyCode)
  ...
  then
    elt##.onmousedown :=
      Dom_html.handler (fun _ev ->
          ...
          button := true ;
          let s =
            {mouse_x; mouse_y; button= true; keypressed= false; key= null}
          in
          f s ; Js._true ) ;
  ...
  then
    elt##.onmouseup :=
      Dom_html.handler (fun _ev ->
          ...
          button := false ;
          let s =
            {mouse_x; mouse_y; button= false; keypressed= false; key= null}
          in
          f s ; Js._true ) ;
  elt##.onmousemove :=
    Dom_html.handler (fun ev ->
        ...
        mouse_y :=
          elt##.height - (Js.Optdef.get ev##.pageY (fun _ -> 0) - cy) ;
        ...
        then
          ...
          f s ) ;
        Js._true ) ;
  (* EventListener sur le doc car pas de moyen simple de le faire sur un
     canvasElement *)
  ...
  then
    doc##.onkeypress :=
      Dom_html.handler (fun ev ->
          ...
          let mouse_x, mouse_y = get_pos_mouse () in
          let s =
            {mouse_x; mouse_y; button= !button; keypressed= true; key}
          in
          f s ; Js._true )
