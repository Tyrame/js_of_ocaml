...
module Color = struct
  ...
  type t =
    ...
    | RGB_percent of (int * int * int)
        (**RGB channels are specified as a percentage of their maximal
           value.*)
    | RGBA of (int * int * int * float)
        (**Same as RGB with additionnal transparency argument. Opacity
           should be in [0.] (completely transparent) and [1.] (completely
           opaque).*)
  ...
  let rgb ?a r g b =
    match a with None -> RGB (r, g, b) | Some a -> RGBA (r, g, b, a)

  let hsl ?a h s l =
    match a with None -> HSL (h, s, l) | Some a -> HSLA (h, s, l, a)
  ...
  let hex_of_rgb (red, green, blue) =
    let in_range i =
      ...
      then
        raise
          (Invalid_argument (string_of_int i ^ " is out of valid range"))
    in
    in_range red ;
    in_range green ;
    in_range blue ;
  ...
  let js_t_of_js_string s =
    ...
    then s
    else if List.mem (Js.to_string s)
              [ "aliceblue"
              ; "antiquewhite"
              ; "aqua"
              ; "aquamarine"
  ...
  let js = function
    | Name n -> name n
    | (RGB _ | RGB_percent _ | RGBA _ | RGBA_percent _ | HSL _ | HSLA _) as
      c ->
  ...
  let ml c =
    ...
    try Name (name_of_string s) with Invalid_argument _ ->
      let fail () =
        raise (Invalid_argument (s ^ " is not a valid color"))
      in
      let re_rgb =
        Regexp.regexp
          "(rgba?)\\((?:(\\d*),(\\d*),(\\d*)(?:,(\\d*(?:\\.\\d*)?))?)\\)"
      ...
      let i_of_s_o = function
        ...
        | Some i ->
          ...
          | Invalid_argument s | Failure s ->
              raise
                (Invalid_argument
                   ("color conversion error (" ^ i ^ "): " ^ s))
      in
      let f_of_s f =
        try float_of_string f with
        | Invalid_argument s | Failure s ->
            raise
              (Invalid_argument ("color conversion error (" ^ f ^ "): " ^ s))
      in
      ...
      | Some r -> (
          ...
          | Some "rgba" -> (
            ...
            | None -> fail ()
            | Some a ->
                RGBA (i_of_s_o red, i_of_s_o green, i_of_s_o blue, f_of_s a)
            )
          | Some _ | None -> fail () )
      | None ->
        ...
        | Some r -> (
            ...
            | Some "rgb" -> (
              ...
              | Some _ -> fail ()
              | None ->
                  RGB_percent (i_of_s_o red, i_of_s_o green, i_of_s_o blue)
              )
            | Some "rgba" -> (
              match alpha with
              | None -> fail ()
              | Some a ->
                  RGBA_percent
                    (i_of_s_o red, i_of_s_o green, i_of_s_o blue, f_of_s a)
              )
            | Some _ | None -> fail () )
        | None ->
...
module Length = struct
  ...
  (*TODO: type js_t = private Js.string Js.t val js_t_of_t val t_of_js_t val
    t_of_string *)
  ...
  let ml t =
    ...
    then Zero
      ...
      let fail () =
        raise (Invalid_argument (s ^ " is not a valid length"))
      in
